...
class DealController extends Controller
{
    ...
    public function flash_delete(Request $request,$id)
    {
        ...
        $flashdeal->status = "0" ;
        $flashdeal->save();
        Toastr::success(Helpers::translate('deleted successfully!'));
        return back();
    }
    ...
    public function flash_submit(Request $request)
    {
        $flash_deal_id = DB::table('flash_deals')->insertGetId([
            'title' => $request['title'][array_search('sa', $request->lang)],
            'start_date' => $request['start_date'],
            'end_date' => $request['end_date'],
            'background_color' => $request['background_color'],
            'text_color' => $request['text_color'],
            'banner' => $request->has('image') ? ImageManager::upload('deal/', 'png', $request->file('image')) : 'def.png',
            'slug' => Str::slug($request['title'][array_search('sa', $request->lang)]),
        ...
        Toastr::success(Helpers::translate('Deal added successfully!'));
        return back();
    }
    ...
    public function update(Request $request, $deal_id)
    {
        ...
        Toastr::success(Helpers::translate('Deal updated successfully!'));
        return back();
    }
    ...
    {
        if($request->status){
            if(!Helpers::module_permission_check('marketing.deal.flash.enable')){
                Toastr::error( Helpers::translate('You do not have access'));
                return back();
            }
        }else{
            if(!Helpers::module_permission_check('marketing.deal.flash.disable')){
                Toastr::error( Helpers::translate('You do not have access'));
                return back();
            }
        }
    ...
    {
        if($request->status){
            if(!Helpers::module_permission_check('marketing.deal.feature.enable')){
                return 0;
            }
        }else{
            if(!Helpers::module_permission_check('marketing.deal.feature.disable')){
                return 0;
            }
        }
    ...
    public function feature_index(Request $request)
    {
        ...
        if ($request->has('search')) {
            ...
            $flash_deals = FlashDeal::where('deal_type', 'feature_deal')->where('deleted',0);
        }
    ...
    public function add_product_submit(Request $request, $deal_id)
    {
        ...
        if(!isset($flash_deal_products))
        {
            ...
            Toastr::success(Helpers::translate('Product added successfully!'));
            return back();
        }else{
    ...
    public function deal_of_day_submit(Request $request)
    {
        ...
        if($deal_id) {
            foreach ($request->lang as $index => $key) {
                if ($request->title[$index] && $key != 'nolang') {
                    Translation::updateOrInsert(
    ...
    {
        if($request->status){
            if(!Helpers::module_permission_check('marketing.deal.day.enable')){
                return 0;
            }
        }else{
            if(!Helpers::module_permission_check('marketing.deal.day.disable')){
                return 0;
            }
        }
