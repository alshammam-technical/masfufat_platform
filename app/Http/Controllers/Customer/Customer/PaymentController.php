...
class PaymentController extends Controller
{
    public function payment(Request $request)
    {
        ...
        if ($validator->fails()) { //api
            ...
            if(in_array($request->payment_request_from, ['app', 'react'])){
                ...
                foreach ($errors as $value) {
                    Toastr::error(Helpers::translate($value['message']));
                }
                return back();
            }
        }
        ...
        if(!$product_stock && in_array($request->payment_request_from, ['app', 'react'])){
            ...
            Toastr::error(Helpers::translate('the_following_items_in_your_cart_are_currently_out_of_stock'));
    ...
    public function customer_payment_request(Request $request)
    {
        ...
        if(in_array($request->payment_request_from, ['app', 'react'])) {
            ...
            $payment_amount = Helpers::order_summary_user_auth()['total'];
        }else{
        ...
        if(isset($request['order_id'])){
            ...
            $userMobile = str_replace('+966','',$user['phone']);
            $userMobile = str_replace('966','',$userMobile);
            $ot = Helpers::get_order_totals($order);
            if((Order::where('order_group_id',$order->order_group_id)->first()->id == $order->id) || ($order->shipping_tax)){
                $payment_amount = $ot['total'];
            }else{
                $payment_amount = $ot['total'] - $ot['shipping'] - ($order['shipping_tax'] * ($ot['shipping']/100));
            }
        }
        ...
        $payment_info = new PaymentInfo(
            ...
            attribute: 'order',
            attribute_id: $request['order_id'] ?? '10001'
        );
    ...
    {
        $uid = Helpers::get_customer()['id'];
        $storeId = session('user_type') == 'delegate' ? session('original_store_id') : auth('customer')->id() ;
        $order = Order::where('customer_id',$storeId)->orderBy('id','desc')->first() ?? Order::where('customer_id',$uid)->orderBy('id','desc')->first();
        return redirect('/account-order-details?id='.$order['id']);
    ...
    public function customer_add_to_fund_request(Request $request)
    {
        if(Helpers::get_business_settings('add_funds_to_wallet') != 1)
        {
            ...
            Toastr::error(Helpers::translate('add_funds_to_wallet_is_deactivated'));
            return back();
        }
        ...
        if(!(Helpers::currency_converter($request->amount, $current_currency) >= Helpers::currency_converter($minimum_add_fund_amount, 'SAR')) || !(Helpers::currency_converter($request->amount, $current_currency) <= Helpers::currency_converter($maximum_add_fund_amount, 'SAR')))
        {
            ...
            if(in_array($request->payment_request_from, ['app', 'react'])){
                ...
                Toastr::error(Helpers::translate('the_amount_needs_to_be_between').' '.Helpers::currency_converter($minimum_add_fund_amount).' - '.Helpers::currency_converter($maximum_add_fund_amount));
                // return back();
            }
        }
    ...
    public function customer_subscription_request(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'payment_method' => 'required',
            'payment_platform' => 'required',
        ...
        $value = Package::find($request['subscription'])->price;
        session()->put('plan_id',$request['subscription']);
        $payment_info = new PaymentInfo(
            ...
            receiver_id: '100',
            additional_data: $additional_data,
            payment_amount: Helpers::currency_converter($value, $current_currency),
            external_redirect_link: $request->payment_platform == 'web' ? $request->external_redirect_link : null,
    ...
    public function web_payment_success(Request $request)
    {
        if($request->flag == 'success') {
            if (session()->has('payment_mode') && session('payment_mode') == 'app') {
                ...
                Toastr::success(Helpers::translate('Payment_success'));
            ...
            if(session()->has('payment_mode') && session('payment_mode') == 'app'){
                ...
                Toastr::error(Helpers::translate('Payment_failed').'!');
                return redirect(url('/'));
            }
        }
