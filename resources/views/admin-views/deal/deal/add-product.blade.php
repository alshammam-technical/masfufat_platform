...
@section('title', \App\CPU\Helpers::translate('Deal Product'))
@push('css_or_js')
...
<div class="content container-fluid">
    ...
    <div class="mb-3">
        <h2 class="h1 mb-0 text-capitalize">
            ...
            {{\App\CPU\Helpers::translate('Add a product to the list')}}
        </h2>
    </div>
    <!-- End Page Title -->
                ...
                <div class="card-body">
                    <form action="{{route('admin.deal.add-product',[$deal['id']])}}" method="post">
                        ...
                        <div class="form-group">
                            <div class="row">
                                <div class="col-md-12">
                                    <label for="name" class="title-color text-capitalize">{{ \App\CPU\Helpers::translate('choose the product')}}</label>
                                    <select
                                        ...
                                        @foreach (\App\Model\Product::active()->orderBy('name', 'asc')->get() as $key => $product)
                                            <option value="{{ $product->id }}">
                                                {{\App\CPU\Helpers::getItemName('products','name',$product['id'],$lang ?? 'sa') ?? $product['name']}}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
                        ...
                        <div class="d-flex justify-content-end">
                            <button type="submit" class="btn bg-primaryColor btn-primary px-4 bg-primaryColor"">{{ \App\CPU\Helpers::translate('add')}}</button>
                        </div>
                    </form>
                </div>
            </div>
            ...
            <div class="card">
                <div class="px-3 py-4">
                    <h5 class="mb-0 text-capitalize">
                        {{ \App\CPU\Helpers::translate('products in the list')}}
                    ...
                    <table class="table table-hover table-borderless table-thead-bordered table-nowrap table-align-middle card-table w-full" cellspacing="0">
                        ...
                        @foreach($products as $k=>$de_p)
                            <tr>
                                ...
                                <td><a href="{{route('admin.product.edit',['id'=>$de_p['id']])}}" target="_blank" class="font-weight-semibold title-color hover-c1">{{$de_p['name']}}</a></td>
                                <td>{{($de_p['unit_price'])}}</td>
...
@push('script')
    ...
    <script>
        $(document).on('click', '.delete', function () {
            var id = $(this).attr("id");
            Swal.fire({
                title: "{{\App\CPU\Helpers::translate('Are_you_sure_remove_this_product')}}?",
                ...
                if (result.value) {
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            toastr.success('{{\App\CPU\Helpers::translate('product_removed_successfully')}}');
                            location.reload();
                        }
                    });
                }
