...
@section('title', \App\CPU\Helpers::translate('articles'))

@push('css_or_js')
    <meta name="csrf-token" content="{{ csrf_token() }}">
    <style>
        .btnn {
        background-color: transbarent; /* اللون الأساسي للزر */
        transition: background-color 0.3s; /* يضيف تأثير انتقالي للون */
    }
    .btnn:hover {
        background-color: #fdcd05; /* اللون عند تمرير المؤشر فوق الزر */
    }
...
@section('content')
    <div class="content container-fluid">
        ...
        <div class="row">
                    ...
                    <nav aria-label="breadcrumb" style="flex-grow: 1;width: 100%;">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item" aria-current="page">
                                <a>{{\App\CPU\Helpers::translate('articles')}}</a>
                            </li>
                        </ol>
                    </nav>
            ...
            <div class="col-lg-7" hidden>
                <div style="display:none" class="d-flex table-actions flex-wrap justify-content-end mx-3">
                    <div class="d-flex">
                    @if (\App\CPU\Helpers::module_permission_check('education_articles_stores.add'))
                    <a title="{{Helpers::translate('Add new')}}" class="btn btn-success my-2 btn-icon-text m-2"
                    href="{{route('admin.store-education.back-end.articles.add')}}">
                        <i class="fa fa-plus"></i>
                    </a>
                    @endif
                    @if (\App\CPU\Helpers::module_permission_check('education_articles_stores.delete'))
                    <button title="{{Helpers::translate('delete')}}" class=" btn btn-danger my-2 btn-icon-text m-2"
                    onclick="form_alert('articles-bulk-delete','Are you sure ?')"
                    >
        ...
        <div class="card">
            ...
            <div class="table-responsive datatable-custom">
                <table
                    ...
                    <tr>
                        ...
                        <th>{{\App\CPU\Helpers::translate('Article')}}</th>
                        <th>{{\App\CPU\Helpers::translate('Category')}}</th>
                        <th>{{\App\CPU\Helpers::translate('Published date')}}</th>
                        @if (\App\CPU\Helpers::module_permission_check('education_articles_stores.enable'))
                    ...
                    @foreach($articles as $key=>$article)
                        <tr>
                            <td>
                                ...
                                action="{{route('admin.customer.update',['id'=>$article['id'],'bool_r'=>1])}}"
                                enctype="multipart/form-data"
                                >
                                @csrf
                                <input type="checkbox" class="trSelector" onchange="handleRowSelect(this)">
                                <span class="rowId" hidden>{{$article->id}}</span>
                            </td>
                            <td>
                                {{$key + 1}}
                            </td>
                            <td><a class="title-color hover-c1 d-flex align-items-center gap-10 spanValue"
                                href="{{ route('admin.store-education.back-end.articles.edit', $article->id) }}" style="justify-content: center;">{{ $article->title }}</a>
                            </td>

                            <td><a href="{{ route('admin.store-education.back-end.categories.edit', $article->category->id ?? '') }}" ><span
                                class="title-color hover-c1 d-flex align-items-center gap-10 spanValue" style="justify-content: center;">{{ $article->category->name ?? \App\CPU\Helpers::translate('The section does not exist or has been deleted')}}</span></a></td>

                                <td><a href="#"><span class="title-color hover-c1 d-flex align-items-center gap-10 spanValue" style="justify-content: center;">{{ $article->created_at }}</span></a></td>
                                @if (\App\CPU\Helpers::module_permission_check('education_articles_stores.enable'))
                                <td>
                                    <center>
                                        <label class="switch switch-status mx-1">
                                            <input type="checkbox"
                                            id="{{$article->id}}" class="switcher_input article-status" {{$article->status == 1?'checked':''}}>
                                ...
                                <td>
                                    <div class="d-flex justify-content-center gap-2">
                                        <a title="{{\App\CPU\Helpers::translate('View')}}"
                                           ...
                                           href="{{ route('admin.store-education.article', $article->slug) }}">
                                        ...
                                        @if (\App\CPU\Helpers::module_permission_check('education_articles_stores.edit'))
                                        <a title="{{\App\CPU\Helpers::translate('Edit')}}"
                                           class="btn btn-white border-0"
                                           href="{{ route('admin.store-education.back-end.articles.edit', [$article['id']]) }}">
                                        ...
                                        <a title="{{\App\CPU\Helpers::translate('Delete')}}"
                                           class="btn btn-white border-0 delete-article" data-id="{{ $article['id'] }}" title="{{\App\CPU\Helpers::translate('Delete')}}"
            ...
            <form hidden action="{{route('admin.store-education.back-end.articles-bulk-delete')}}" method="post" id="articles-bulk-delete">
                @csrf
                @method('delete')
...
@push('script_2')
    <script>
        $(document).ready(function(){
            $('.delete-article').click(function(){
                ...
                var url = "{{ route('admin.store-education.back-end.articles.delete') }}";
    ...
    <script>
        $(document).on('change', '.article-status', function () {
            var id = $(this).attr("id");
            ...
            $.ajax({
                url: "{{route('admin.education.back-end.articles.status')}}",
                method: 'POST',
                data: {
                    id: id,
                    status: status
