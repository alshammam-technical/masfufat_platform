...
@section('content')
                ...
                @forelse($translates as $translate)
                    <div class="vironeer-translated-item d-block d-lg-flex bd-highlight align-items-center">
                        <div class="flex-grow-1 bd-highlight">
                            <textarea id="autosizeInput" class="vironeer-translate-key translate-fields form-control"
                                rows="1" readonly>{{ $translate->key }}</textarea>
                        </div>
                        <div class="pe-3 ps-3 bd-highlight text-center text-success d-none d-lg-block"><i
                                class="fas fa-chevron-right"></i></div>
                        <div class="flex-grow-1 bd-highlight">
                            <textarea id="autosizeInput" name="values[{{ $translate->id }}]"
                                class="translate-fields form-control" rows="1">{{ $translate->value }}</textarea>
                        </div>
                    </div>
                @empty
    ...
    @push('scripts_libs')
        <script src="{{ asset('/public/assets/vendor/libs/autosize/autosize.min.js') }}"></script>
    @endpush
    @push('scripts')
        <script>
            autosize($('textarea'));
