...
@section('title', \App\CPU\Helpers::translate(Ticket', 'tickets') . ' #' . $ticket->ticket_number)
@section('back', route('user.tickets'))
@section('content')
    @if ($ticket->status == 3)
        <div class="note note-warning">
            <strong>{{ \App\CPU\Helpers::translate(Note', 'tickets') }} : </strong>
            {{ \App\CPU\Helpers::translate(Ticket has been closed you can sent a reply to reopen it', 'tickets') }}
        </div>
    @endif
                ...
                @foreach ($replies as $reply)
                    @if ($reply->admin_id == null)
                        <li class="vr__chat__message d-flex justify-content-between">
                            <div class="custom-card card bg-white text-dark w-full">
                                <div class="card-body">
                                    <div class="d-flex justify-content-between">
                                            ...
                                            @foreach ($reply->supportAttachments as $attachment)
                                                <a href="{{ route('user.tickets.download', [$ticket->ticket_number, encrypt($attachment->id)]) }}"
                                                    class="btn btn-dark btn-sm mb-2 me-1"><i
                                                        class="fa fa-download me-1"></i>{{ \App\CPU\Helpers::translate(Attachment', 'tickets') }}</a>
                                            @endforeach
                                        </div>
                                    @endif
                                </div>
                    ...
                    @else
                        <li class="vr__chat__message d-flex justify-content-between">
                            ...
                            <div class="custom-card card w-full bg-blue text-white">
                                <div class="card-body">
                                    <div class="d-flex justify-content-between">
                                        <p class="fw-bold">
    ...
    <div class="card custom-card dash__forms">
        <div class="card-body">
            <form action="{{ route('user.tickets.reply', $ticket->ticket_number) }}" method="POST"
                ...
                @csrf
                <div class="mb-3">
                    <label class="form-label">{{ \App\CPU\Helpers::translate(Reply message', 'tickets') }} : <span
