...
<div class="d-flex product justify-content-between inline_product" style="cursor: pointer;"
    ...
    <div class="product_details {{$product['current_stock']==0?'stock-card':''}}">
        <h3 class="product-title">
            <a class="ptr ptr1" href="{{route('product',$product->slug)}}">{{Helpers::getItemName('products','name',$product->id)}}</a>
        </h3>
        ...
        @if($product['current_stock']<=0)
            <label class="badge badge-danger stock-out-side">{{\App\CPU\Helpers::translate('Stock Out')}}</label>
        @endif
    </div>
</div>
