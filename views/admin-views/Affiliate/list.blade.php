...
@section('title', \App\CPU\Helpers::translate('Affiliate List'))
@push('css_or_js')
@endpush
            ...
            <div class="col-lg-5">
                <nav aria-label="breadcrumb" style="width:100%; text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item" aria-current="page">{{\App\CPU\Helpers::translate('Affiliate')}}</li>
                    </ol>
                </nav>
            </div>
            ...
            <div class="col-md-12">
                <div class="card" style="width:100%; text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};">
                    <div class="card-body">
                        <div class="table-responsive">
                            <table style="text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};"
                                ...
                                <tr>
                                    ...
                                    <th>{{ Helpers::translate('marketer_name') }}</th>
                                    <th>{{ Helpers::translate('marketer_email') }}</th>
                                    <th>{{ Helpers::translate('marketer_city') }}</th>
                                    <th>{{ Helpers::translate('notes') }}</th>
                                    <th>{{ Helpers::translate('commission_type') }}</th>
                                    <th>{{ Helpers::translate('amount') }} / {{ Helpers::translate('percent') }}</th>
                                    <th>{{ Helpers::translate('apply_to') }}</th>
                                    <th>{{ Helpers::translate('show_total_sales') }}</th>
                                ...
                                <tbody>
                                @foreach($Affiliate as $k=>$b)
                                    <tr>
                                        <form class="table-editor-form" method="POST" action="{{route('admin.Affiliate.update',[$b['id']])}}">
                                            <td>
                                                @csrf
                                                <input type="checkbox" class="trSelector" onchange="handleRowSelect(this)">
                                                <span class="rowId" hidden>{{$b->id}}</span>
                                            </td>
                                            <td class="text-center">{{$Affiliate->firstItem()+$k}}</td>
                                            ...
                                            <td>
                                                <span class="spanValue">
                                                    {{ Helpers::translate($b['commission_type']) }}
                                                </span>
                                                <input
                                                class="form-control editValue"
                                                type="text"
                                                value="{{ $b['commission_type'] }}"
                                                name="commission_type">
                                            </td>
                                            <td>
                                                <span class="spanValue">
                                                    {{ ($b['commission_type'] == 'static') ? $b['amount'] : $b['percent'] }}
                                                </span>
                                                <input
                                                class="form-control editValue"
                                                type="text"
                                                value="{{ ($b['commission_type'] == 'static') ? $b['amount'] : $b['percent'] }}"
                                                name="{{ ($b['commission_type'] == 'static') ? 'amount' : 'percent' }}">
                                            </td>
                                            <td>
                                                <span class="spanValue">
                                                    {{ Helpers::translate($b['apply_to']) }}
                                                </span>
                                                <input
                                                class="form-control editValue"
                                                type="text"
                                            ...
                                            <td>
                                                ...
                                                href="{{route('admin.Affiliate.update',[$b['id']])}}">
                                                    <i class="tio-edit"></i>
                                                </a>
                                            </td>
                                        </form>
                        ...
                        <form hidden action="{{route('admin.Affiliate.bulk-delete')}}" method="post" id="bulk-delete">
                            @csrf @method('delete')
                            <input type="text" name="ids" class="ids">
                            <input type="text" name="not_ids" class="not_ids">
                        </form>

                        <form hidden action="{{route('admin.Affiliate.bulk-status',['status'=>true])}}" method="post" id="bulk-enable">
                            @csrf @method('post')
                            <input type="text" name="ids" class="ids">
                            <input type="text" name="not_ids" class="not_ids">
                        </form>

                        <form hidden action="{{route('admin.Affiliate.bulk-status',['status'=>false])}}" method="post" id="bulk-disable">
                            @csrf @method('post')
                            <input type="text" name="ids" class="ids">
                            <input type="text" name="not_ids" class="not_ids">
                        </form>
