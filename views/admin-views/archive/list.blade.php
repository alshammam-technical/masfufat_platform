...
@section('title', \App\CPU\Helpers::translate('Archive'))
@push('css_or_js')
@endpush
        ...
        <div class="row" style="margin-top: 20px">
            <div class="col-lg-5">
                <nav aria-label="breadcrumb" style="width:100%; text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item" aria-current="page">{{\App\CPU\Helpers::translate('Archive')}}</li>
                    </ol>
                </nav>
            </div>
            ...
            <div class="col-lg-7" hidden>
                <div class="d-flex table-actions flex-wrap justify-content-end mx-3">
                    <div class="d-flex">
                    <a title="{{Helpers::translate('Add new')}}" class="btn ti-plus btn-success my-2 btn-icon-text m-2 disabled">
                        ...
                        <div class="table-responsive">
                            <table style="text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};"
                                ...
                                @foreach($items as $k=>$b)
                                    <tr>
                                        <form class="table-editor-form" method="POST" action="{{route('admin.archives.update',[$b['id']])}}">
                                            <td>
                                                @csrf
                                                <input type="checkbox" class="trSelector" onchange="handleRowSelect(this)">
                                                <span class="rowId" hidden>{{$b->id}}</span>
                                            </td>
                                            <td class="text-center">{{$archive->firstItem()+$k}}</td>
                                            @foreach(\App\CPU\Helpers::get_langs() as $lang)
                                            <td>
                                                <span class="spanValue">
                                                    {{ \App\CPU\Helpers::get_prop('App\archive',$b['id'],'name',$lang) ?? $b['name'] }}
                                                </span>
                                                <input
                                                class="form-control editValue lang_value"
                                                type="text"
                                                value="{{ \App\CPU\Helpers::get_prop('App\archive',$b['id'],'name',$lang) ?? $b['name'] }}"
                                                name="name[]"
                                            ...
                                            <td>
                                                ...
                                                href="{{route('admin.archives.update',[$b['id']])}}">
                                                    <i class="tio-edit"></i>
                                                </a>
                                            </td>
                                        </form>
