...
@section('title', \App\CPU\Helpers::translate('areas List'))
@push('css_or_js')
@endpush
        ...
        <div class="row" style="margin-top: 20px">
            ...
            <div class="col-lg-7" hidden>
                <div class="d-flex table-actions flex-wrap justify-content-end mx-3">
                    <div class="d-flex">
                    <a title="{{Helpers::translate('Add new')}}" class="btn ti-plus btn-success my-2 btn-icon-text m-2" href="{{route('admin.areas.add-new')}}">
                        <i class="fa fa-plus"></i>
                    </a>
                    <button title="{{Helpers::translate('Add from')}}" class="btn btn-info mdi mdi-arrange-bring-forward btnAddFrom my-2 btn-icon-text m-2" disabled="disabled" onclick="addFrom('areas')">
                        ...
                        <div class="dropdown dropdown">
                            ...
                            <div aria-labelledby="droprightMenuButton" class="dropdown-menu">
                                ...
                                <div aria-labelledby="droprightMenuButton" class="dropdown-menu tx-13" style="position: absolute;will-change: transform;top: -10%;left: -100%;transform: translate3d(0px, 145px, 0px);">
                                    ...
                                    <a class="dropdown-item bulk-export" href="{{route('admin.areas.bulk-export')}}">
                                        {{\App\CPU\Helpers::translate('export to excel')}}
                                    </a>
                                    <a class="dropdown-item bulk-import" href="{{route('admin.areas.bulk-import')}}">
                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <table style="text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};"
                                ...
                                <tbody>
                                @foreach($areas as $k=>$b)
                                    <tr>
                                        <form class="table-editor-form" method="POST" action="{{route('admin.areas.update',[$b['id']])}}">
                                            <td>
                                                @csrf
                                                <input type="checkbox" class="trSelector" onchange="handleRowSelect(this)">
                                                <span class="rowId" hidden>{{$b->id}}</span>
                                            </td>
                                            <td class="text-center">{{$areas->firstItem()+$k}}</td>
                                            @foreach(\App\CPU\Helpers::get_langs() as $lang)
                                            <td>
                                                <span class="spanValue">
                                                    {{ \App\CPU\Helpers::get_prop('App\areas',$b['id'],'name',$lang) ?? $b['name'] }}
                                                </span>
                                                <input
                                                class="form-control editValue lang_value"
                                                type="text"
                                                value="{{ \App\CPU\Helpers::get_prop('App\areas',$b['id'],'name',$lang) ?? $b['name'] }}"
                                                name="name[]"
                                            ...
                                            <td>
                                                <span class="spanValue">
                                                    {{ \App\CPU\Helpers::get_prop('App\countries',$b['parent_id'],'name') ?? $b['name'] }}
                                                </span>
                                            ...
                                            <td>
                                                ...
                                                href="{{route('admin.areas.update',[$b['id']])}}">
                                                    <i class="tio-edit"></i>
                                                </a>
                                            </td>
                                        </form>
                        ...
                        <form hidden action="{{route('admin.areas.bulk-delete')}}" method="post" id="bulk-delete">
                            @csrf @method('delete')
                            <input type="text" name="ids" class="ids">
                            <input type="text" name="not_ids" class="not_ids">
                        </form>

                        <form hidden action="{{route('admin.areas.bulk-status',['status'=>true])}}" method="post" id="bulk-enable">
                            @csrf @method('post')
                            <input type="text" name="ids" class="ids">
                            <input type="text" name="not_ids" class="not_ids">
                        </form>

                        <form hidden action="{{route('admin.areas.bulk-status',['status'=>false])}}" method="post" id="bulk-disable">
                            @csrf @method('post')
                            <input type="text" name="ids" class="ids">
                            <input type="text" name="not_ids" class="not_ids">
                        </form>
...
@push('script')
    <script>
        $(document).on('change', '.switcher_input', function () {
            ...
            $.ajax({
                url: "{{route('admin.areas.bulk-status')}}",
                method: 'POST',
                data: {
                    ids: id,
                    status: status,
