...
@section('title', \App\CPU\Helpers::translate('Mail Config'))
@push('css_or_js')
    ...
    <div class="content container-fluid" style="padding-{{(Session::get('direction') == 'rtl') ? 'right' : 'left'}}: 24%">
        ...
        <div class="mb-4 pb-2">
            <h2 class="h1 mb-0 text-capitalize d-flex align-items-center gap-2">
                ...
                {{\App\CPU\Helpers::translate('Mail Config')}}
            </h2>
        </div>
        <!-- End Page Title -->
        ...
        <div class="row" style="text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};">
            @if(Helpers::module_permission_check('mail-module.test'))
            <div class="col-12 mb-4">
                <div class="card">
                    <div class="card-body">
                        <div class="row align-items-center">
                            <div class="col-10">
                                <button class="btn btn--primary btn-primary" type="button" data-toggle="collapse"
                                        data-target="#collapseExample" aria-expanded="false"
                                        aria-controls="collapseExample">
                                    <i class="tio-email-outlined"></i>
                                    {{\App\CPU\Helpers::translate('test_your_email_integration')}}
                                </button>
                            </div>
                                        ...
                                        <div class="col-8">
                                            <div class="form-group mb-2">
                                                <label for="inputPassword2"
                                                       class="sr-only">{{\App\CPU\Helpers::translate('mail')}}</label>
                                                <input type="email" id="test-email" class="form-control"
                                                       placeholder="Ex : jhon@email.com">
                                            </div>
                                        </div>
                                        <div class="col-4">
                                            <button type="button" onclick="send_mail()" class="btn btn--primary btn-primary mb-2 btn-block">
                                                <i class="tio-telegram"></i>
                                                {{\App\CPU\Helpers::translate('send_mail')}}
                                            </button>
                                        </div>
                                    </div>
                                </form>
            ...
            <div class="col-sm-6 mb-3 mb-sm-0">
                <div class="card h-100">
                    <div class="card-header">
                        <h5 class="mb-0">{{\App\CPU\Helpers::translate('smtp_mail_config')}}</h5>
                    </div>
                    <div class="card-body">
                        ...
                        <form action="{{route('admin.business-settings.mail.update')}}"
                            ...
                            @if(isset($data_smtp))
                                ...
                                <label class="mb-3 d-block title-color">{{\App\CPU\Helpers::translate('smtp_mail')}}</label>
                                ...
                                <div class="form-group">
                                    <label class="title-color">{{\App\CPU\Helpers::translate('mailer_name')}}</label><br>
                                    <input type="text" placeholder="ex : Alex" class="form-control" name="name"
                                           value="{{env('APP_MODE')=='demo'?'':$data_smtp['name']}}">
                                </div>

                                <div class="form-group">
                                    <label class="title-color">{{\App\CPU\Helpers::translate('Host')}}</label><br>
                                    <input type="text" class="form-control" name="host"
                                           value="{{env('APP_MODE')=='demo'?'':$data_smtp['host']}}">
                                </div>
                                <div class="form-group">
                                    <label class="title-color">{{\App\CPU\Helpers::translate('Driver')}}</label><br>
                                    <input type="text" class="form-control" name="driver"
                                           value="{{env('APP_MODE')=='demo'?'':$data_smtp['driver']}}">
                                </div>
                                <div class="form-group">
                                    <label class="title-color">{{\App\CPU\Helpers::translate('port')}}</label><br>
                                    <input type="text" class="form-control" name="port"
                                           value="{{env('APP_MODE')=='demo'?'':$data_smtp['port']}}">
                                </div>

                                <div class="form-group">
                                    <label class="title-color">{{\App\CPU\Helpers::translate('Username')}}</label><br>
                                ...
                                <div class="d-flex flex-wrap justify-content-end gap-10">
                                    ...
                                    @else
                                        <button type="submit"
                                                class="btn btn--primary btn-primary px-4">{{\App\CPU\Helpers::translate('Configure')}}</button>
                                    @endif
                                </div>
                        </form>
                    </div>
            ...
            <div class="col-sm-6">
                <div class="card h-100">
                    <div class="card-header">
                        <h5 class="mb-0">{{\App\CPU\Helpers::translate('sendgrid_mail_config')}}</h5>
                    </div>
                    <div class="card-body">
                        ...
                        <form action="{{route('admin.business-settings.mail.update-sendgrid')}}"
                            ...
                            <label class="mb-3 d-block title-color">{{\App\CPU\Helpers::translate('sendgrid_mail')}}</label>
                            ...
                            <div class="form-group">
                                <label class="title-color">{{\App\CPU\Helpers::translate('mailer_name')}}</label>
                                <input type="text" placeholder="ex : Alex" class="form-control" name="name"
                                        value="{{env('APP_MODE')=='demo'?'':$data_sendgrid['name']}}">
                            </div>

                            <div class="form-group">
                                <label class="title-color">{{\App\CPU\Helpers::translate('Host')}}</label>
                                <input type="text" class="form-control" name="host"
                                        value="{{env('APP_MODE')=='demo'?'':$data_sendgrid['host']}}">
                            </div>
                            <div class="form-group">
                                <label class="title-color">{{\App\CPU\Helpers::translate('Driver')}}</label>
                                <input type="text" class="form-control" name="driver"
                                        value="{{env('APP_MODE')=='demo'?'':$data_sendgrid['driver']}}">
                            </div>
                            <div class="form-group">
                                <label class="title-color">{{\App\CPU\Helpers::translate('port')}}</label>
                                <input type="text" class="form-control" name="port"
                                        value="{{env('APP_MODE')=='demo'?'':$data_sendgrid['port']}}">
                            </div>

                            <div class="form-group">
                                <label class="title-color">{{\App\CPU\Helpers::translate('Username')}}</label>
                                <input type="text" placeholder="ex : ex@yahoo.com" class="form-control"
                                        name="username"
                            ...
                            <div class="form-group">
                                <label class="title-color">{{\App\CPU\Helpers::translate('email_id')}}</label>
                                <input type="text" placeholder="ex : ex@yahoo.com" class="form-control" name="email"
                                        value="{{env('APP_MODE')=='demo'?'':$data_sendgrid['email_id']}}">
                            </div>

                            <div class="form-group">
                                <label class="title-color">{{\App\CPU\Helpers::translate('Encryption')}}</label>
                                <input type="text" placeholder="ex : tls" class="form-control" name="encryption"
        ...
        function send_mail() {
            if (ValidateEmail($('#test-email').val())) {
                Swal.fire({
                    ...
                    text: "{{\App\CPU\Helpers::translate('a_test_mail_will_be_sent_to_your_email')}}!",
                    ...
                    if (result.value) {
                        ...
                        $.ajax({
                            ...
                            success: function (data) {
                                if (data.success === 2) {
                                    toastr.error('{{\App\CPU\Helpers::translate('email_configuration_error')}} !!');
                                } else if (data.success === 1) {
                                    toastr.success('{{\App\CPU\Helpers::translate('email_configured_perfectly!')}}!');
                                } else {
                                    toastr.info('{{\App\CPU\Helpers::translate('email_status_is_not_active')}}!');
                                }
                            },
                            complete: function () {
                                $('#loading').hide();
                ...
                toastr.error('{{\App\CPU\Helpers::translate('invalid_email_address')}} !!');
            }
        }
    </script>
@endpush
