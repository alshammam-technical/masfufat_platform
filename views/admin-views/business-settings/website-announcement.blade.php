...
@section('title', \App\CPU\Helpers::translate('announcement'))
                ...
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item" aria-current="page">
                            <a>
                                {{Helpers::translate('Announcement_Setup')}}
                            </a>
                        </li>
                    </ol>
                </nav>
            ...
            @if (isset($announcement))
                <div class="card h-100">
                    <div class="card-header">
                        <h5 class="mb-0">{{\App\CPU\Helpers::translate('announcement_setup')}} ({{Helpers::translate('It appears on the website and the app')}})</h5>
                    </div>
                    <div class="card-body">
                        <div class="d-flex gap-10 align-items-center mb-2">
                            ...
                            <label class="title-color mb-0">{{\App\CPU\Helpers::translate('Enable')}}</label>
                        </div>
                        <div class="d-flex gap-10 align-items-center mb-4">
                            <input type="radio" name="announcement_status"
                                    value="0" {{$announcement['status']==0?'checked':''}}>
                            <label class="title-color mb-0">{{\App\CPU\Helpers::translate('Disable')}}</label>
                        </div>
                        ...
                        <div class="form-group">
                            <label class="title-color d-flex">{{\App\CPU\Helpers::translate('announcement text')}}</label>
                            <input class="form-control" type="text" name="announcement"
                                    value="{{ $announcement['announcement'] }}">
                        </div>

                        <div class="justify-content-end d-flex">
                            <button type="submit" class="btn btn--primary btn-primary px-4">{{ Helpers::translate('Save') }}</button>
                        </div>
                    </div>
                </div>
            @endif
