...
<html>
  <head>
    <title>{{ Helpers::translate('Import Categories') }}</title>
    <meta charset="UTF-8" />
  ...
  <body>
    ...
    <script>
        ...
        const columns = [
            ...
            {
                "prop": "name[{{$index}}]",
                "name": "{{Helpers::translate('Category_Name').'('.Helpers::translate($lang.' language').')'}}",
                "size": 300
            },
            @endforeach
            {
                "editor": "checkbox",
                "prop": "is_sub",
                "name": "{{Helpers::translate('is sub category')}}",
                "size": 300
            },
            {
                "prop": "parent_id",
            ...
            },
            {
                ...
                "name": "{{Helpers::translate('Show category for pricing levels')}}",
                "size": 300,
                "editor": 'select',
            ...
            },
            {
                "editor": "checkbox",
                "prop": "home_status",
                "name": "{{Helpers::translate('home_status')}}",
                "size": 300
            },
            @foreach(Helpers::get_langs() as $index => $lang)
            {
            ...
            {
                "prop": "icon[]",
                "name": "{{Helpers::translate('Icon (upload)').' ('.Helpers::translate($lang.' language').')'}}",
                "editor": "img",
                "size": 300
            },
            {
            ...
            {
                "prop": "iconUrl[{{$lang}}][]",
                "name": "{{Helpers::translate('Icon (url)').' ('.Helpers::translate($lang.' language').')'}}",
                "editor": "textarea",
                "size": 300
            },
            @endforeach
        ...
        async function save(){
            ...
            for (let key = current; key <= (parseInt(source_length) + 1); key++) {
                ...
                if(item['priority']){
                    ...
                    await $.ajax({
                        url: "{{ route('admin.category.bulk-import-ii') }}",
                        type: 'post',
                        data: formData[key],
                        dataType: 'json',
                        contentType: false,
                        ...
                        success: function(data){
                            if(data.errors){
                                ...
                                if(row == source_length){
                                    location.replace("{{route('admin.category.view')}}")
                                }
                            }
                        },
                        error: function(){
