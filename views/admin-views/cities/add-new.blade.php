...
@section('title', \App\CPU\Helpers::translate('city Add'))
                ...
                <div class="dropdown dropdown">
                    ...
                    <div aria-labelledby="droprightMenuButton" class="dropdown-menu">
                        ...
                        <div aria-labelledby="droprightMenuButton" class="dropdown-menu tx-13" style="position: absolute;will-change: transform;top: -10%;left: -100%;transform: translate3d(0px, 145px, 0px);">
                            ...
                            <a class="dropdown-item" href="{{route('admin.countries.bulk-import')}}">
        ...
        <div class="col-md-12">
            <div class="card mb-3">
                <div class="card-body" style="text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};">
                    <form action="{{route('admin.cities.add-new')}}" method="post" enctype="multipart/form-data">
                        @csrf
                        @php
                                ...
                                @foreach(Helpers::get_langs() as $lang)
                                    <div class="form-group {{$lang != $default_lang ? 'd-none':''}} lang_form"
                                        ...
                                        <label for="name" class="title-color">
                                            <img class="ml-2" width="20" src="{{asset('public/assets/front-end')}}/img/flags/{{$lang}}.png">
                                            {{ \App\CPU\Helpers::translate('city_Name')}}<span class="text-danger">*</span> ({{strtoupper($lang)}})
                                        ...
                                        type="text" name="name[]" class="form-control theName" id="name" value="" placeholder="{{\App\CPU\Helpers::translate('Ex')}} : {{\App\CPU\Helpers::translate('LUX')}}" {{$lang == ($default_lang ?? session()->get('local')) ? 'required':''}}>
                                    </div>
                        ...
                        <div class="d-flex gap-3 justify-content-end" hidden>
                            <button type="submit" hidden class="btn-save btn btn--primary btn-primary px-4">{{ \App\CPU\Helpers::translate('submit')}}</button>
                        </div>
                    </form>
                </div>
            </div>
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    $.ajax({
                        ...
                        data: {id: id},
                        success: function () {
                            toastr.success('{{\App\CPU\Helpers::translate('city_deleted_successfully')}}');
                            location.reload();
                        }
                    });
                }
