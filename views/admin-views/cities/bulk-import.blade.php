...
<html>
  <head>
    <title>{{ Helpers::translate('Import cities') }}</title>
    <meta charset="UTF-8" />
    ...
    <script>
        ...
        const columns = [
            ...
            @endforeach
            {
                ...
                "editor": 'select',
                "single": 1,
                "editorSource": {!! Helpers::getAreasOptions() !!},
            },
            {
                "editor": "checkbox",
                "prop": "enabled",
        ...
        async function save(){
            ...
            for (let key = current; key <= (parseInt(source_length) + 1); key++) {
                ...
                if(item['name[0]']){
                    ...
                    await $.ajax({
                        url: "{{ route('admin.cities.bulk-import-ii') }}",
                        type: 'post',
                        data: formData[key],
                        dataType: 'json',
                        contentType: false,
                        ...
                        success: function(data){
                            if(data.errors){
                                ...
                                if(row == source_length){
                                    location.replace("{{route('admin.cities.list')}}")
                                }
                            }
                        },
                        error: function(){
