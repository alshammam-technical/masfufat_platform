...
@section('title', \App\CPU\Helpers::translate('country Add'))
        ...
        <div class="col-md-12">
            <div class="card mb-3">
                <div class="card-body" style="text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};">
                    <form action="{{route('admin.countries.add-new')}}" method="post" enctype="multipart/form-data">
                        @csrf
                        @php
                                ...
                                @foreach(Helpers::get_langs() as $lang)
                                    <div class="form-group {{$lang != $default_lang ? 'd-none':''}} lang_form"
                                        ...
                                        <label for="name" class="title-color">
                                            <img class="ml-2" width="20" src="{{asset('public/assets/front-end')}}/img/flags/{{$lang}}.png">
                                            {{ \App\CPU\Helpers::translate('country_Name')}}<span class="text-danger">*</span> ({{strtoupper($lang)}})
                                        <a class="btn btn-primary" onclick="emptyInput(event,'.card-body','.the-name')">{{\App\CPU\Helpers::translate('Field dump')}}</a>
                                        </label>
                                        <input type="text" name="name[]" class="form-control the-name" id="name" value="" placeholder="{{\App\CPU\Helpers::translate('Ex')}} : {{\App\CPU\Helpers::translate('LUX')}}" {{$lang == ($default_lang ?? session()->get('local')) ? 'required':''}}
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group" id="{{$lang}}-form">
                                    <label class="title-color" for="name">{{ \App\CPU\Helpers::translate('country Code') }}
                                    </label>
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    $.ajax({
                        ...
                        data: {id: id},
                        success: function () {
                            toastr.success('{{\App\CPU\Helpers::translate('country_deleted_successfully')}}');
                            location.reload();
                        }
                    });
                }
