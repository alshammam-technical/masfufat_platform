...
@section('title', \App\CPU\Helpers::translate('Countries List'))
@push('css_or_js')
@endpush
        ...
        <div class="row" style="margin-top: 20px">
            ...
            <div class="col-lg-7" hidden>
                <div class="d-flex table-actions flex-wrap justify-content-end mx-3">
                    ...
                    <button title="{{Helpers::translate('Add from')}}" class="btn btn-info mdi mdi-arrange-bring-forward btnAddFrom my-2 btn-icon-text m-2" disabled="disabled" onclick="addFrom('countries')">
                            ...
                            <table style="text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};"
                                ...
                                <tr>
                                    ...
                                    <th scope="col" style="width: 100px" class="text-center">
                                        {{ \App\CPU\Helpers::translate('country code')}}
                                    </th>
                                    <th scope="col" style="width: 100px" class="text-center">
                                ...
                                <tbody>
                                @foreach($countries as $k=>$b)
                                    <tr>
                                        <form class="table-editor-form" method="POST" action="{{route('admin.countries.update',[$b['id']])}}">
                                            <td>
                                                @csrf
                                                <input type="checkbox" class="trSelector" onchange="handleRowSelect(this)">
                                                <span class="rowId" hidden>{{$b->id}}</span>
                                            </td>
                                            <td class="text-center">{{$countries->firstItem()+$k}}</td>
                                            @foreach(\App\CPU\Helpers::get_langs() as $lang)
                                            <td>
                                                <span class="spanValue">
                                                    {{ \App\CPU\Helpers::get_prop('App\countries',$b['id'],'name',$lang) ?? $b['name'] }}
                                                </span>
                                                <input
                                                class="form-control editValue lang_value"
                                                onchange="translateName(event,'tr','input[name=\'name[]\']')"
                                                type="text"
                                                value="{{ \App\CPU\Helpers::get_prop('App\countries',$b['id'],'name',$lang) ?? $b['name'] }}"
                                                name="name[]">
                                            ...
                                            <td>
                                                ...
                                                href="{{route('admin.countries.update',[$b['id']])}}">
                                                    <i class="tio-edit"></i>
                                                </a>
                                            </td>
                                        </form>
