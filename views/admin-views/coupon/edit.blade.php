...
@section('title', \App\CPU\Helpers::translate('Coupon Edit'))
@push('css_or_js')
                ...
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item" aria-current="page">
                            <a>
                                {{Helpers::translate('coupon_update')}}
                            </a>
                        </li>
                    </ol>
                </nav>
                    ...
                    <form action="{{route('admin.coupon.update',[$c['id']])}}" method="post">
                        ...
                        <div class="row">
                            <div class="col-md-6 col-lg-4 form-group">
                                <label for="name" class="title-color text-capitalize">{{\App\CPU\Helpers::translate('coupon_type')}}</label>
                                <select class="form-control" id="coupon_type" name="coupon_type" required>
                                    <option disabled selected>{{\App\CPU\Helpers::translate('Select Coupon Type')}}</option>
                                    <option value="discount_on_purchase" {{$c['coupon_type']=='discount_on_purchase'?'selected':''}}>{{\App\CPU\Helpers::translate('Discount on Purchase')}}</option>
                                    <option value="free_delivery" {{$c['coupon_type']=='free_delivery'?'selected':''}}>{{\App\CPU\Helpers::translate('Free_Delivery')}}</option>
                                    <option value="first_order" {{$c['coupon_type']=='first_order'?'selected':''}}>{{\App\CPU\Helpers::translate('First_Order')}}</option>
                                </select>
                            </div>
                            <div class="col-md-6 col-lg-4 form-group">
                                <label for="name" class="title-color text-capitalize">{{\App\CPU\Helpers::translate('coupon_title')}}</label>
                                <input type="text" name="title" class="form-control" id="title" value="{{$c['title']}}"
                                       placeholder="{{\App\CPU\Helpers::translate('Title')}}" required>
                            </div>
                            <div class="col-md-6 col-lg-4 form-group">
                                <label for="name" class="title-color text-capitalize">{{\App\CPU\Helpers::translate('coupon_code')}}</label>
                                <a href="javascript:void(0)" class="float-right" onclick="generateCode()">{{\App\CPU\Helpers::translate('generate_code')}}</a>
                            ...
                            <div class="col-md-6 col-lg-4 form-group first_order">
                                <label for="name" class="title-color font-weight-medium d-flex">{{\App\CPU\Helpers::translate('coupon_bearer')}}</label>
                                <select class="form-control" name="coupon_bearer" id="coupon_bearer" >
                                    <option disabled selected>{{\App\CPU\Helpers::translate('Select_coupon_bearer')}}</option>
                                    <option value="seller" {{$c['coupon_bearer']=='seller'?'selected':''}}>{{\App\CPU\Helpers::translate('seller')}}</option>
                                    <option value="inhouse" {{$c['coupon_bearer']=='inhouse'?'selected':''}}>{{\App\CPU\Helpers::translate('admin')}}</option>
                                </select>
                            </div>
                            <div class="col-md-6 col-lg-4 form-group coupon_by first_order">
                                ...
                                <select class="js-example-basic-multiple js-states js-example-responsive form-control" name="seller_id" id="seller_wise_coupon">
                                    <option disabled selected>{{\App\CPU\Helpers::translate('select_seller')}}</option>
                                    <option value="0" {{$c['seller_id']=='0'?'selected':''}}>{{\App\CPU\Helpers::translate('all_seller')}}</option>
                                    @if($c['coupon_bearer'] == 'inhouse')
                                    <option value="inhouse" {{is_null($c['seller_id'])?'selected':''}}>{{\App\CPU\Helpers::translate('inhouse')}}</option>
                                    @endif
                            ...
                            <div class="col-md-6 col-lg-4 form-group coupon_type first_order">
                                ...
                                <select class="js-example-basic-multiple js-states js-example-responsive form-control" name="customer_id" >
                                    <option disabled selected>{{\App\CPU\Helpers::translate('Select_customer')}}</option>
                                    <option value="0" {{$c['customer_id']=='0'?'selected':''}}>{{\App\CPU\Helpers::translate('all_customer')}}</option>
                            ...
                            <div class="col-md-6 col-lg-4 form-group first_order">
                                <label  for="exampleFormControlInput1" class="title-color text-capitalize">{{\App\CPU\Helpers::translate('limit for same user')}}</label>
                                <input type="number" name="limit" min="0" value="{{ $c['limit'] }}" id="coupon_limit" class="form-control" placeholder="{{\App\CPU\Helpers::translate('EX')}}: 10">
                            </div>
                            <div class="col-md-6 col-lg-4 form-group free_delivery">
                                <label  for="name" class="title-color text-capitalize">{{\App\CPU\Helpers::translate('discount_type')}}</label>
                                <select id="discount_type" class="form-control" name="discount_type"
                                        onchange="checkDiscountType(this.value)">
                                    <option value="amount" {{$c['discount_type']=='amount'?'selected':''}}>{{\App\CPU\Helpers::translate('Amount')}}</option>
                                    <option value="percentage" {{$c['discount_type']=='percentage'?'selected':''}}>{{\App\CPU\Helpers::translate('percentage')}}</option>
                                </select>
                            </div>
                            <div class="col-md-6 col-lg-4 form-group free_delivery">
                                <label for="name" class="title-color text-capitalize">{{\App\CPU\Helpers::translate('Discount_Amount')}} <span id="discount_percent"> (%)</span></label>
                                <input type="number" min="0" max="1000000" step=".01" name="discount" class="form-control" id="discount" value="{{$c['discount_type']=='amount'?\App\CPU\Convert::default($c['discount']):$c['discount']}}"
                                    placeholder="{{\App\CPU\Helpers::translate('Ex: 500')}}" required>
                            </div>
                            <div class="col-md-6 col-lg-4 form-group">
                                <label for="name" class="title-color text-capitalize">{{\App\CPU\Helpers::translate('minimum_purchase')}}</label>
                                <input type="number" min="0" max="1000000" step=".01" name="min_purchase" class="form-control" id="minimum purchase" value="{{\App\CPU\Convert::default($c['min_purchase'])}}"
                                       placeholder="{{\App\CPU\Helpers::translate('minimum purchase')}}" required>
                            </div>
                            <div class="col-md-6 col-lg-4 form-group free_delivery" id="max-discount">
                                <label for="name" class="title-color text-capitalize">{{\App\CPU\Helpers::translate('maximum_discount')}}</label>
                                <input type="number" min="0" max="1000000" step=".01" name="max_discount" class="form-control" id="maximum discount" value="{{\App\CPU\Convert::default($c['max_discount'])}}"
                                       placeholder="{{\App\CPU\Helpers::translate('maximum discount')}}">
                            </div>
                            <div class="col-md-6 col-lg-4 form-group">
                            ...
                            <div class="col-md-6 col-lg-4 form-group">
                                <label for="name" class="title-color text-capitalize">{{\App\CPU\Helpers::translate('expire_date')}}</label>
                        ...
                        <div class="d-flex align-items-center justify-content-end flex-wrap gap-10">
                            ...
                            <button type="submit" class="btn btn--primary px-4">{{\App\CPU\Helpers::translate('Update')}}</button>
                        </div>
                    </form>
                </div>
            </div>
