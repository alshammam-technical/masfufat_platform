<div class="modal-header">
    <h5 class="modal-title" id="exampleModalLongTitle">
        {{ \App\CPU\Helpers::translate('History_of_Order_No:') }} {{ $histories[0]->order_id?? '##' }}
        ...
        <div class="timeline-steps">
            ...
            <div class="timeline-step {{ $history->status == 'returned' || $history->status == 'failed' || $history->status == 'canceled'? 'failed' : 'completed' }}">
                ...
                <div class="timeline-info">
                    ...
                    <p class="timeline-text">{{ \App\CPU\Helpers::translate('Service_Time') }} : {{$history->created_at->format('d/m/y')}} {{$history->created_at->format('h:i A')}}</p>
                </div>
            </div>
            @empty
            <div class="timeline-step failed">
                ...
                <div class="timeline-info">
                    <p class="timeline-title">{{ \App\CPU\Helpers::translate('No_history_for_this_order') }}</p>
                </div>
            </div>
            @endforelse
