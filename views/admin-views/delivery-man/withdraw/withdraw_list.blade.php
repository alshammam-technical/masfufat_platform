...
@section('title', \App\CPU\Helpers::translate('deliverymen wallet balances'))
    ...
    <div class="content container-fluid">
        ...
        <div class="mb-3">
            <h2 class="h1 mb-0 text-capitalize d-flex align-items-center gap-2">
                ...
                {{\App\CPU\Helpers::translate('deliverymen wallet balances')}}
            </h2>
        </div>
        <!-- End Page Title -->
                ...
                <div class="card">
                    <div class="px-3 py-4">
                        <div class="row align-items-center">
                            <div class="col-lg-4">
                                <h5>
                                    {{ \App\CPU\Helpers::translate('deliverymen Withdraw Request Table')}}
                            ...
                            <div class="col-lg-8 mt-3 mt-lg-0 d-flex gap-3 justify-content-lg-end">
                                ...
                                <select name="delivery_withdraw_status_filter" onchange="status_filter(this.value)" class="custom-select min-w-120 max-w-200">
                                    <option value="all" {{session()->has('delivery_withdraw_status_filter') && session('delivery_withdraw_status_filter') == 'all'?'selected':''}}>{{\App\CPU\Helpers::translate('All')}}</option>
                                    <option value="approved" {{session()->has('delivery_withdraw_status_filter') && session('delivery_withdraw_status_filter') == 'approved'?'selected':''}}>{{\App\CPU\Helpers::translate('Approved requests')}}</option>
                                    <option value="denied" {{session()->has('delivery_withdraw_status_filter') && session('delivery_withdraw_status_filter') == 'denied'?'selected':''}}>{{\App\CPU\Helpers::translate('Denied requests')}}</option>
                                    <option value="pending" {{session()->has('delivery_withdraw_status_filter') && session('delivery_withdraw_status_filter') == 'pending'?'selected':''}}>{{\App\CPU\Helpers::translate('Pending requests')}}</option>
                                </select>
                    ...
                    <div class="table-responsive">
                        <table id="datatable"
                            ...
                            <tr>
                                ...
                                <th>{{ \App\CPU\Helpers::translate('deliveryman name') }}</th>
                                <th>{{\App\CPU\Helpers::translate('request_time')}}</th>
                                <th class="text-center">{{\App\CPU\Helpers::translate('request status')}}</th>
                            ...
                            @foreach($withdraw_req as $k=>$wr)
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        @else
                                        <span>{{\App\CPU\Helpers::translate('not_found')}}</span>
                                        @endif
                                    </td>
                                    <td>{{ date_format( $wr->created_at, 'Y/m/d H:i:') }}</td>
                                    <td class="text-center">
                                        @if($wr->approved==0)
                                            <label class="badge badge-soft-primary">{{\App\CPU\Helpers::translate('Pending')}}</label>
                                        @elseif($wr->approved==1)
                                            <label class="badge badge-soft-success">{{\App\CPU\Helpers::translate('Approved')}}</label>
                                        @else
                                            <label class="badge badge-soft-danger">{{\App\CPU\Helpers::translate('Denied')}}</label>
                                        @endif
                                    </td>
                                    <td>
