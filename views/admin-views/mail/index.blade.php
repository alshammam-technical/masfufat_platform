...
@section('title', \App\CPU\Helpers::translate('Add new mail'))
...
@section('content')
    <div class="content container-fluid">
        ...
        <div class="row gx-2 gx-lg-3">
            @if(Helpers::module_permission_check('marketing.mail.add'))
                    ...
                    <div class="card-body">
                        <form action="{{route('admin.mail.store')}}" method="post"
                                ...
                                <div class="col-md-12">
                                    <div class="form-group">
                                        <label class="title-color text-capitalize"
                                               for="exampleFormControlInput1">{{\App\CPU\Helpers::translate('mail text')}} </label>

                                        <div class="form-group">
                                            <textarea class="form-control textarea_mail" id="textarea_mail"
                                    ...
                                    <div class="input-group">
                                        ...
                                        <div class="d-block gap-10 align-items-center mb-2 w-100">
                                                        ...
                                                        onchange="getRequest('{{url('/')}}/admin/product/get-categories?parent_id='+this.value,'sub-category-select','select')">
                                                            @foreach(\App\Model\Seller::all() as $user)
                                                                <option value="{{$user['id']}}">
                                                                    ...
                                                                    {{ $user->store_informations['email'] ?? $user['email'] }} ,
                                            ...
                                            <div class="mx-2 mb-3">
                                                ...
                                                <label class="title-color mb-0">{{\App\CPU\Helpers::translate('an external email')}}</label>

                                                <div class="input-group" style="display: none">
                                                    <input class="form-control" type="email" name="sent_to[ext]">
                                                </div>
                                            </div>
                            ...
                            <div class="d-flex justify-content-end gap-3">
                                <button type="submit" class="btn btn--primary btn-primary">{{\App\CPU\Helpers::translate('Send mail')}}  </button>
                            </div>
                        </form>
                    </div>
                </div>
                        ...
                        <div class="row align-items-center">
                            <div class="col-sm-4 col-md-6 col-lg-8 mb-2 mb-sm-0">
                                <h5 class="mb-0 text-capitalize d-flex align-items-center gap-2">
                                    {{ \App\CPU\Helpers::translate('Push_mail_Table')}}
                                    <span
                                        class="badge badge-soft-dark radius-50 fz-12 ml-1">{{ $mails->total() }}</span>
                                </h5>
                            </div>
                                ...
                                <form action="{{ url()->current() }}" method="GET">
                                    <div class="input-group input-group-merge input-group-custom">
                                        ...
                                        <input id="datatableSearch_" type="search" name="search" class="form-control"
                                               placeholder="{{\App\CPU\Helpers::translate('Search by email text')}}"
                    ...
                    <div class="table-responsive datatable-custom">
                        <table style="text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};"
                            ...
                            <tr>
                                ...
                                <th>{{\App\CPU\Helpers::translate('mail text')}} </th>
                            ...
                            @foreach($mails as $key=>$mail)
                                <tr>
                                    ...
                                    <td>
                                        {{\Illuminate\Support\Str::limit($mail['description'],40)}}
                                    </td>
                                    <td>
                                        <a href="javascript:void(0)" class="btn btn-outline-success square-btn btn-sm"
                                           onclick="resendmail(this)" data-id="{{ $mail->id }}">
...
@push('script_2')
    <script>
        ...
        function resendmail(t) {
            let id = $(t).data('id');

            Swal.fire({
                title: '{{\App\CPU\Helpers::translate("Are_you_sure?")}}',
                text: '{{\App\CPU\Helpers::translate('Resend_mail')}}',
                type: 'warning',
                ...
                if (result.value) {
                    $.ajax({
                        url: '{{ route("admin.mail.resend-mail") }}',
                        type: 'POST',
                        data: {
                            _token: '{{ csrf_token() }}',
                            id: id
                        ...
                        success: function (res) {
                            CKEDITOR.instances['textarea_mail'].setData(res.description)
                        },
                        complete: function () {
                            $('#loading').hide();
                        }
    ...
    <script>
        var editor_mail = $('.textarea_mail').ckeditor({
            contentsLangDirection : '{{Session::get('direction')}}',
        });
        CKEDITOR.instances['textarea_mail'].on("blur", function(e) {
        ...
        function emptyDesc(){
            CKEDITOR.instances['textarea_mail'].setData('')
        }
    </script>
    {{--ck editor--}}
@endpush
