            ...
            <div class="col-md-12">
                <div class="card" style="width:100%; text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};">
                    <div class="card-body">
                            ...
                            <table style="text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};"
                                ...
                                <tr>
                                    ...
                                    <th scope="col" style="width: 100px">
                                        {{ \App\CPU\Helpers::translate('package ID')}}
                                    </th>
                                    @foreach(\App\CPU\Helpers::get_langs() as $lang)
                                    <th scope="col">{{ \App\CPU\Helpers::translate('name')}} ({{$lang}})</th>
                                    @endforeach
                                    <th scope="col">{{ \App\CPU\Helpers::translate('package type')}}</th>
                                    <th scope="col">{{ \App\CPU\Helpers::translate('package period')}}</th>
                                    <th scope="col">{{ \App\CPU\Helpers::translate('price')}}</th>
                                ...
                                @foreach($br as $k=>$b)
                                    <tr>
                                        <form class="table-editor-form" method="POST" action="{{route('admin.package.update',[$b['id']])}}">
                                            <td>
                                                @csrf
                                            ...
                                            <td>
                                                <span class="spanValue">
                                                    {{ \App\CPU\Helpers::get_prop('App\Package',$b['id'],'name',$lang) ?? $name }}
                                                </span>
                                                <input class="form-control editValue lang_value" type="text"
                                                value="{{ \App\CPU\Helpers::get_prop('App\Package',$b['id'],'name',$lang) ?? $name }}"
                                                name="name[]"
                                            ...
                                            <td>
                                                <span class="spanValue">
                                                    {{\App\CPU\BackEndHelper::set_symbol(($b['price']))}}
                                                </span>
                                                <input class="form-control editValue" type="text"
                                                value="{{\App\CPU\Convert::default($b['price'])}}"
                                                name="price">
                                            </td>
                                            <td>
                                                <a class="btn btn-primary btn-sm" title="{{ \App\CPU\Helpers::translate('Edit')}}"
                                                href="{{route('admin.package.update',[$b['id']])}}">
                                                    <i class="tio-edit"></i>
                                                </a>
                                            </td>
                                        </form>
                        ...
                        <form hidden action="{{route('admin.package.bulk-status',['status'=>true])}}" method="post" id="bulk-enable">
                            @csrf @method('post')
                            <input type="text" name="ids" class="ids">
                            <input type="text" name="not_ids" class="not_ids">
                        </form>

                        <form hidden action="{{route('admin.package.bulk-status',['status'=>false])}}" method="post" id="bulk-disable">
                            @csrf @method('post')
                            <input type="text" name="ids" class="ids">
                            <input type="text" name="not_ids" class="not_ids">
                        </form>
