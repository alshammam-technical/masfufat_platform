...
@section('title', \App\CPU\Helpers::translate('POS Order List'))
    ...
    <div class="content container-fluid">
        ...
        <!-- <div class="page-header mb-1">
            <div class="flex-between align-items-center">
                <div>
                    <h1 class="page-header-title">{{\App\CPU\Helpers::translate('pos_orders')}} <span
        ...
        <div class="mb-3">
            <h2 class="h1 mb-0 text-capitalize d-flex flex-wrap align-items-center gap-2">
                ...
                {{\App\CPU\Helpers::translate('POS_Orders')}}
                        ...
                        <form action="" id="form-data" method="GET">
                            <div class="d-flex justify-content-end flex-wrap flex-md-nowrap gap-3">
                                ...
                                <input type="date" value="{{$to}}" name="to" id="to_date"
                                        class="form-control">
                                <button type="submit" class="btn btn--primary btn-primary" onclick="formUrlChange(this)" data-action="{{ url()->current() }}">
                    ...
                    @foreach($orders as $key=>$order)
                        <tr class="status-{{$order['order_status']}} class-all">
                            ...
                            <td>
                                ...
                                @else
                                    <label class="badge badge-danger">{{\App\CPU\Helpers::translate('invalid_customer_data')}}</label>
                                @endif
                            </td>
                            <td>
                                @if($order->payment_status=='paid')
                                    <span class="badge badge-soft-success">{{\App\CPU\Helpers::translate('paid')}}
                                    </span>
                                @else
                                    <span class="badge badge-soft-danger">{{\App\CPU\Helpers::translate('unpaid')}}
                                    </span>
                                @endif
                            </td>
                            ...
                            <td>
                                <div class="d-flex justify-content-center gap-2">
                                    <a class="btn btn-outline--primary btn-sm" title="{{\App\CPU\Helpers::translate('view')}}"
                                        href="{{route('admin.pos.order-details',['id'=>$order['id']])}}">
                                        <i class="tio-invisible"></i>
                                    </a>
                                    <a class="btn btn-outline-info btn-sm" target="_blank" title="{{\App\CPU\Helpers::translate('invoice')}}"
