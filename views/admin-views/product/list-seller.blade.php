        ...
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a
                        href="{{route('admin.dashboard')}}">{{\App\CPU\Helpers::translate('Dashboard')}}</a></li>
                @if($pro['data'] != null && $pro->first()->request_status == 0)
                    <li class="breadcrumb-item"
                        aria-current="page">{{\App\CPU\Helpers::translate('New Products')}}</li>
                @elseif($pro['data'] != null && $pro->first()->request_status == 1)
                    <li class="breadcrumb-item"
                        aria-current="page">{{\App\CPU\Helpers::translate('Approved Products')}}</li>
                @elseif($pro['data'] != null && $pro->first()->request_status == 2)
                    <li class="breadcrumb-item"
                        aria-current="page">{{\App\CPU\Helpers::translate('Denied Products')}}</li>
                @else
                ...
                <div class="card">
                    @if($pro->first() != null && $pro->first()->added_by == 'in_house')
                        <div class="card-header">
                            <h5>{{\App\CPU\Helpers::translate('product_table')}}</h5>
                            <a href="{{route('admin.product.add-new')}}" class="btn btn--primary btn-primary  float-right">
                                <i class="tio-add-circle"></i>
                                <span class="text">{{\App\CPU\Helpers::translate('Add new product')}}</span>
                            </a>
                        </div>
                    @endif
                    <div class="card-body" style="padding: 0">
                        <div class="table-responsive">
                            <table id="datatable"
                                ...
                                <tr>
                                    <th style="width:50px !important">{{\App\CPU\Helpers::translate('SL#')}}</th>
                                    ...
                                    <th>{{\App\CPU\Helpers::translate('verify_status')}}</th>
                                ...
                                @foreach($pro as $k=>$p)
                                    <tr>
                                        <th style="width:50px !important" scope="row">{{$k+1}}</th>
                                        <td>
                                        ...
                                        <td>
                                            @if($p->request_status == 0)
                                                <label class="badge badge-warning">{{\App\CPU\Helpers::translate('New Request')}}</label>
                                            @elseif($p->request_status == 1)
                                                <label class="badge badge-success">{{\App\CPU\Helpers::translate('Approved')}}</label>
                                            @elseif($p->request_status == 2)
                                                <label class="badge badge-danger">{{\App\CPU\Helpers::translate('Denied')}}</label>
                                            @endif
                                        </td>
                                        ...
                                        <td>
                                            <a class="btn btn--primary btn-primary btn-sm"
                                               href="{{route('admin.product.edit',[$p['id']])}}">
                                                <i class="tio-edit"></i>{{\App\CPU\Helpers::translate('Edit')}}
                                            </a>
                                            <a class="btn btn-danger btn-sm" href="javascript:"
                                               onclick="form_alert('product-{{$p['id']}}','Want to delete this item ?')">
                                                <i class="tio-add-to-trash"></i> {{\App\CPU\Helpers::translate('Delete')}}
                                            </a>
