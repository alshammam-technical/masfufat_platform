...
<html>
  <head>
    <title>{{ Helpers::translate('Import provinces') }}</title>
    <meta charset="UTF-8" />
  ...
  <body>
    ...
    <script>
        ...
        const columns = [
            ...
            {
                "prop": "parent_id",
                "name": "{{Helpers::translate('belongs to')}}",
                "size": 300,
                "editor": 'select',
                "single": 1,
                "editorSource": {!! Helpers::getCitiesOptions() !!},
            },
            {
                "editor": "checkbox",
                "prop": "enabled",
        ...
        async function save(){
            ...
            for (let key = current; key <= (parseInt(source_length) + 1); key++) {
                ...
                if(item['name[0]']){
                    ...
                    await $.ajax({
                        url: "{{ route('admin.provinces.bulk-import-ii') }}",
                        type: 'post',
                        data: formData[key],
                        dataType: 'json',
                        contentType: false,
                        ...
                        success: function(data){
                            if(data.errors){
                                ...
                                if(row == source_length){
                                    location.replace("{{route('admin.provinces.list')}}")
                                }
                            }
                        },
                        error: function(){
