...
@section('title',\Helpers::translate('refund_transactions'))
            ...
            <div class="card-header border-0 px-3 py-4">
                <div class="w-100 d-flex flex-wrap gap-3 align-items-center">
                    <h4 class="mb-0 mr-auto">
                        {{ \Helpers::translate('total_transaction')}}
                    ...
                    <form action="{{ url()->current() }}" method="GET" class="mb-0">
                        ...
                        <div class="input-group input-group-merge input-group-custom">
                            ...
                            <input id="datatableSearch_" type="search" name="search" class="form-control"
                                   placeholder="{{ \Helpers::translate('Search by orders id _or_refund_id')}}" aria-label="Search orders"
                                   value="{{ $search }}">
                    ...
                    <form action="#" id="form-data" method="GET">
                        <div class="d-flex flex-wrap gap-2 align-items-center">
                            <select class="form-control __form-control w-auto" name="payment_method" id="payment_method">
                                <option value="all" {{ $payment_method=='all' ? 'selected': '' }}>{{\Helpers::translate('all')}}</option>
                                <option value="cash" {{ $payment_method=='cash' ? 'selected': '' }}>{{\Helpers::translate('cash')}}</option>
                                <option value="digitally_paid" {{ $payment_method=='digitally_paid' ? 'selected': '' }}>{{\Helpers::translate('digitally_paid')}}</option>
                                <option value="customer_wallet" {{ $payment_method=='customer_wallet' ? 'selected': '' }}>{{\Helpers::translate('customer_wallet')}}</option>
                            </select>
                                ...
                                <ul class="dropdown-menu dropdown-menu-right">
                                    <li>
                                        <a class="dropdown-item"
                                           href="{{ route('admin.transaction.refund-transaction-export-excel', ['payment_method'=>$payment_method, 'search'=>$search]) }}"  >{{\Helpers::translate('Excel')}}</a>
                                    </li>
                                </ul>
                            </div>
                        </div>
            ...
            <div class="table-responsive">
                <table id="datatable"
                    ...
                    <tr>
                        ...
                        <th>{{\Helpers::translate('product')}}</th>
                        <th>{{\Helpers::translate('refund_id')}}</th>
                        <th>{{\Helpers::translate('order_id')}}</th>
                        <th>{{\Helpers::translate('shop_name')}}</th>
                        <th>{{ \Helpers::translate('payment_method') }}</th>
                        <th>{{\Helpers::translate('payment_status')}}</th>
                        <th>{{\Helpers::translate('paid_by')}}</th>
                        <th>{{\Helpers::translate('amount')}}</th>
                        <th class="text-center">{{\Helpers::translate('transaction_type')}}</th>
                    </tr>
                    </thead>
                    <tbody>
                    @foreach ($refund_transactions as $key=>$refund_transaction)
                        <tr class="text-capitalize">
                            ...
                            <td>
                                ...
                                @else
                                    <span>{{\Helpers::translate('not_found')}}</span>
                                @endif
                            </td>
                            ...
                            <td>
                                {{\Helpers::translate(str_replace('_',' ',$refund_transaction->payment_method))}}
                            </td>
                            <td>
                                {{\Helpers::translate(str_replace('_',' ',$refund_transaction->payment_status))}}
                            </td>
                            <td>
                                {{\Helpers::translate($refund_transaction->paid_by)}}
                            </td>
                            <td>
                                {{\App\CPU\Helpers::currency_converter($refund_transaction->amount)}}
                            </td>
                            <td class="text-center">
                                {{\Helpers::translate($refund_transaction->transaction_type == 'Refund' ? 'refunded' : str_replace('_',' ',$refund_transaction->transaction_type))}}
                            </td>
                        </tr>
                    @endforeach
                    </tbody>
