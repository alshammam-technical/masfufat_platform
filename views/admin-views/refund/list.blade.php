...
@section('title',\App\CPU\Helpers::translate('reund_requests'))
...
<div class="content container-fluid">
    ...
    <div class="mb-3">
        <h2 class="h1 mb-0 text-capitalize d-flex align-items-center gap-2">
            ...
            {{\App\CPU\Helpers::translate($status.' Refund_Requests')}}
            ...
            <div class="row justify-content-between align-items-center">
                    ...
                    <form action="{{ url()->current() }}" method="GET">
                        ...
                        <div class="input-group input-group-merge input-group-custom">
                            ...
                            <input id="datatableSearch_" type="search" name="search" class="form-control"
                                    placeholder="{{\App\CPU\Helpers::translate('Search_by_order_id_or_refund_id')}}" aria-label="Search orders" value="{{ $search }}">
                ...
                <div class="col-12 mt-3 col-md-8">
                    <div class="d-flex gap-3 justify-content-md-end">
                        <label class="mb-0"> {{\App\CPU\Helpers::translate('inhouse_orders_only')}} </label>
            ...
            <table class="table table-hover table-borderless table-thead-bordered table-nowrap table-align-middle card-table w-100"
                ...
                <thead class="thead-light thead-50 text-capitalize">
                    <tr>
                        ...
                        <th>{{\App\CPU\Helpers::translate('refund_id')}} </th>
                        <th>{{\App\CPU\Helpers::translate('order_id')}} </th>
                        <th>{{\App\CPU\Helpers::translate('product_info')}}</th>
                        <th>{{\App\CPU\Helpers::translate('customer_info')}}</th>
                        <th class="text-end">{{\App\CPU\Helpers::translate('total_amount')}}</th>
                        <th>{{\App\CPU\Helpers::translate('refund_status')}}</th>
                ...
                @foreach($refund_list as $key=>$refund)
                    ...
                    <tr>
                        ...
                        <td>
                            <a href="{{route('admin.refund-section.refund.details',['id'=>$refund->id])}}" class="title-color hover-c1">
                                {{$refund->id}}
                            </a>
                        </td>
                        <td>
...
{{--                                @dd($refund->product->thumbnail )--}}
                                <div class="d-flex flex-wrap gap-2">
                                    ...
                                    <div class="d-flex flex-column gap-1">
                                        ...
                                        <span class="fz-12">{{\App\CPU\Helpers::translate('QTY')}} : {{ $refund->order_details->qty }}</span>
                                    </div>
                                </div>
                            @else
                        ...
                        <td>
                            ...
                            @else
                                <a href="#" class="title-color hover-c1">
                                    {{\App\CPU\Helpers::translate('customer_not_found')}}
                                </a>
                            @endif
                        </td>
                        <td>
                        ...
                        <td>
                            <div class="d-inline-flex flex-column gap-1">
                                @if($refund->status=='pending')
                                    <span class="badge badge-soft--primary">{{\App\CPU\Helpers::translate($refund->status)}}</span>
                                @elseif($refund->status=='approved')
                                    <span class="badge badge-soft-success">{{\App\CPU\Helpers::translate($refund->status)}}</span>
                                @elseif($refund->status=='rejected')
                                    <span class="badge badge-soft-danger">{{\App\CPU\Helpers::translate($refund->status)}}</span>
                                @else
                                    <span class="badge badge-soft-warning">{{\App\CPU\Helpers::translate($refund->status)}}</span>
                                @endif
                            </div>
                        </td>
                        <td>
