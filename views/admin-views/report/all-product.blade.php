...
@section('title', \Helpers::translate('Product Report'))
    ...
    <div class="content container-fluid">
        ...
        <div class="mb-3">
            <h2 class="h1 mb-0 text-capitalize d-flex gap-2 align-items-center">
                ...
                {{\Helpers::translate('product_report')}}
            </h2>
        </div>
        <!-- End Page Title -->
        ...
        <div class="store-report-content mb-2">
            <div class="left-content">
                ...
                <div class="left-content-card">
                    ...
                    <div class="info">
                        ...
                        <h6 class="subtext">{{\Helpers::translate('Total_Product_Sale')}}</h6>
                    </div>
                </div>
                <div class="left-content-card">
                    ...
                    <div class="info">
                        ...
                        <h6 class="subtext d-flex">
                            {{\Helpers::translate('Total_Discount_Given')}}
                            <span class="ml-2" data-toggle="tooltip" data-placement="top" title="{{\Helpers::translate('product_wise_discounted_amount_will_be_shown_here')}}">
            ...
            <div class="center-chart-area size-lg">
                <div class="center-chart-header">
                    <h3 class="title d-flex">{{\Helpers::translate('Product_Statistics')}}
                        <span class="ml-2" data-toggle="tooltip" data-placement="top" title="{{\Helpers::translate('The_product_report_will_show_based_on_the_product_added_date')}}">
                            <img class="info-img" src="{{asset('/public/assets/back-end/img/info-circle.svg')}}" alt="img">
                        </span>
                    </h3>
                </div>
                ...
                "data": {
                  ...
                  "datasets": [{
                    "label": "{{\Helpers::translate('total_product')}}",
            ...
            <div class="card-header border-0">
                <div class="d-flex flex-wrap w-100 gap-3 align-items-center">
                    <h4 class="mb-0 {{ (session('direction') == 'ltr') ? 'mr' : 'ml' }}-auto">
                        {{\Helpers::translate('Total_Product')}}
                        <span class="badge badge-soft-dark radius-50 fz-12"> {{ $products->total() }}</span>
                    </h4>
                    <form action="#" method="GET" style="width: 320px">
                        <!-- Search -->
                        <div class="input-group input-group-merge input-group-custom">
                            ...
                            <input id="datatableSearch_" type="search" name="search" class="form-control"
                                    placeholder="{{\Helpers::translate('Search by product name')}}" aria-label="Search orders" value="{{ $search }}">
                    ...
                    <div>
                        ...
                        <ul class="dropdown-menu dropdown-menu-right">
                            <li><a class="dropdown-item" href="{{ route('admin.report.all-product-excel', ['seller_id' => request('seller_id'), 'search' => request('search'), 'date_type' => request('date_type'), 'from' => request('from'), 'to' => request('to')]) }}">{{\Helpers::translate('excel')}}</a></li>
                        </ul>
                    </div>
                </div>
            </div>
                ...
                <div class="table-responsive" id="products-table">
                    <table class="table table-hover __table table-borderless table-thead-bordered table-nowrap table-align-middle card-table w-100 {{Session::get('direction') === "rtl" ? 'text-right' : 'text-left'}}">
                        ...
                        <tr>
                            ...
                            <th>
                                {{\Helpers::translate('Product_Name')}}
                            </th>
                            <th>
                                {{\Helpers::translate('Product_Unit_Price')}}
                            </th>
                            <th>
                                {{\Helpers::translate('Total_Amount_Sold')}}
                            </th>
                            <th>
                                {{\Helpers::translate('Total_Quantity_Sold')}}
                            </th>
                            <th>
                                <div class="d-flex">
                                    <span>{{\Helpers::translate('Average_Product_Value')}} </span>
                            ...
                            <th>
                                {{\Helpers::translate('Current_Stock_Amount')}}
                            </th>
                            <th>
                                {{\Helpers::translate('Average_Ratings')}}
                            </th>
                        </tr>
                        </thead>
                        <tbody>
                        @foreach($products as $key=>$product)
                            <tr>
                                ...
                                <td>
                                    {{ $product->product_type == 'digital' ? ($product->status==1 ? \Helpers::translate('available') : \Helpers::translate('not_available')) : $product->current_stock }}
                                </td>
                                <td>
