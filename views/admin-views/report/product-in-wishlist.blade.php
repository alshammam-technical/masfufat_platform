                                ...
                                <div class="col-sm-6 col-md-4 col-lg-3">
                                    <div class="">
                                        <select
                                            class="form-control"
                                            name="sort">
                                            <option value="ASC" {{ $sort == 'ASC' ? 'selected' : '' }}>{{\Helpers::translate('wishlist_sort_by_(low_to_high)')}}</option>
                                            <option value="DESC" {{ $sort == 'DESC' ? 'selected' : '' }}>{{\Helpers::translate('wishlist_sort_by_(high_to_low)')}}</option>
                                        </select>
                                    </div>
                                </div>
                        ...
                        <div class="d-flex flex-wrap w-100 gap-3 align-items-center">
                            ...
                            <div>
                                ...
                                <ul class="dropdown-menu dropdown-menu-right">
                                    <li><a class="dropdown-item" href="{{route('admin.stock.wishlist-product-export', ['seller_id'=>$seller_id, 'sort'=>$sort, 'search'=>$search])}}">{{\Helpers::translate('excel')}}</a></li>
                                </ul>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="table-responsive" id="products-table">
                        <table class="table table-hover table-borderless table-thead-bordered table-nowrap table-align-middle card-table w-100 {{Session::get('direction') === "rtl" ? 'text-right' : 'text-left'}}">
                            ...
                            <tr>
                                ...
                                <th>
                                    {{\Helpers::translate('The date the product was created')}}
                                </th>
                                <th class="text-center">
                                    {{\Helpers::translate('Total in Wishlist')}}
                                </th>
                            </tr>
                            </thead>
                            <tbody>
                            @foreach($products as $key=>$data)
                                <tr>
                                    ...
                                    <td>{{ date('Y/m/d H:i', $data['created_at'] ? strtotime($data['created_at']) : null) }}</td>
