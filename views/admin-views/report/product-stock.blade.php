                    ...
                    <div class="row gx-2 gy-3 align-items-center text-left">
                        <div class="col-sm-6 col-md-3">
                            <select
                                ...
                                <option value="in_house" {{ $seller_id == 'in_house' ? 'selected' : '' }}>{{\Helpers::translate('In-House')}}</option>
                        ...
                        <div class="col-sm-6 col-md-3">
                            <select class="js-select2-custom form-control __form-control" name="category_id" id="cat_id">
                                <option value="all" {{ $category_id == 'all' ? 'selected' : '' }}>{{\Helpers::translate('all_category')}}</option>
                        ...
                        <div class="col-sm-6 col-md-3">
                            <div class="">
                                <select
                                    class="form-control"
                                    name="sort">
                                    <option value="ASC" {{ $sort == 'ASC' ? 'selected' : '' }}>{{\Helpers::translate('stock_sort_by_(low_to_high)')}}</option>
                                    <option value="DESC" {{ $sort == 'DESC' ? 'selected' : '' }}>{{\Helpers::translate('stock_sort_by_(high_to_low)')}}</option>
                                </select>
                            </div>
                        </div>
                        <div class="col-sm-6 col-md-3">
            ...
            <div class="card-header border-0">
                <div class="d-flex flex-wrap w-100 gap-3 align-items-center">
                    <h4 class="mb-0 {{ (session('direction') == 'ltr') ? 'mr' : 'ml' }}-auto">
                        {{\Helpers::translate('Total_Products')}}
                    ...
                    <div>
                        ...
                        <ul class="dropdown-menu dropdown-menu-right">
                            <li><a class="dropdown-item" href="{{ route('admin.stock.product-stock-export', ['sort' => request('sort'), 'category_id' => request('category_id'), 'seller_id' => request('seller_id'), 'search' => request('search')]) }}">{{\Helpers::translate('excel')}}</a></li>
                        </ul>
                    </div>
                </div>
            </div>
                ...
                <div class="table-responsive" id="products-table">
                    <table class="table table-hover table-borderless table-thead-bordered table-nowrap table-align-middle card-table w-100 {{Session::get('direction') === "rtl" ? 'text-right' : 'text-left'}}">
                        ...
                        <tr>
                            ...
                            <th>
                                {{\Helpers::translate('Product Name')}}
                            </th>
                            <th>
                                {{\Helpers::translate('Last Updated Stock')}}
                            </th>
                            <th class="text-center">
                                {{\Helpers::translate('Current_Stock')}}
                            </th>
                            <th class="text-center">
                                {{\Helpers::translate('Status')}}
                            </th>
                        </tr>
                        </thead>
                        <tbody>
                        @foreach($products as $key=>$data)
                            <tr>
                                ...
                                <td>{{ date('Y/m/d H:i', $data['updated_at'] ? strtotime($data['updated_at']) : null) }}</td>
                                <td class="text-center">{{$data['current_stock']}}</td>
                                <td>
                                    <div class="text-center">
                                        @if($data['current_stock'] >= $stock_limit)
                                            <span class="badge __badge badge-soft-success">{{\Helpers::translate('In-Stock')}}</span>
                                        @elseif($data['current_stock']  == 0)
                                            <span class="badge __badge badge-soft-warning">{{\Helpers::translate('Out_of_Stock')}}</span>
                                        @elseif($data['current_stock'] < $stock_limit)
                                            <span class="badge __badge badge-soft--primary">{{\Helpers::translate('Soon_Stock_Out')}}</span>
                                        @endif
                                    </div>
                                </td>
                            </tr>
