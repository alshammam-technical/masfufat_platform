...
<div class="content container-fluid">
    <div class="row align-items-center mb-3">
        <div class="col-sm">
            <h1 class="page-header-title">{{\App\CPU\Helpers::translate('Orders')}} <span
    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-2">
        <h1 class="h3 mb-0 text-black-50">{{\App\CPU\Helpers::translate('Seller')}} : {{$seller['name']}} , {{\App\CPU\Helpers::translate('ID')}} : {{$seller['id']}}</h1>
    </div>
        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    <h5>{{\App\CPU\Helpers::translate('Order Table')}}</h5>
                </div>
                <div class="card-body">
                        ...
                        <table class="table table-bordered"  width="100%" cellspacing="0">
                            <thead class="thead-light">
                                <tr>
                                    ...
                                    <th>{{\App\CPU\Helpers::translate('Payment')}}</th>

                                    <th style="width: 30px">{{\App\CPU\Helpers::translate('Action')}}</th>
                                </tr>
                                </thead>
                            <tbody>
                            @foreach($orders as $k=>$order)
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        @else
                                            <label class='badge badge-warning'>{{\App\CPU\Helpers::translate('Customer not available')}}</label>
                                        @endif
                                    </td>
                                    <td>
                                        @if($order->customer != null)
                                  ...
                                  <span class="legend-indicator bg-success"></span>{{\App\CPU\Helpers::translate('Paid')}}
                                </span>
                                        @else
                                            <span class="badge badge-soft-danger">
                                  <span class="legend-indicator bg-danger"></span>{{\App\CPU\Helpers::translate('Unpaid')}}
                                </span>
                                        @endif
                                    </td>
                                    <td>
                                            ...
                                            <div class="dropdown-menu" aria-labelledby="dropdownMenuButton" style="margin-top: 0px !important">
                                                <a class="dropdown-item"
                                                   href="{{route('admin.sellers.order-details',[$order['id'],$seller['id']])}}"><i
                                                        class="tio-visible"></i> {{\App\CPU\Helpers::translate('View')}}</a>
                                                <a class="dropdown-item" target="_blank"
                                                   href="{{route('admin.orders.generate-invoice',[$order->id])}}"><i
                                                        class="tio-download"></i> {{\App\CPU\Helpers::translate('Invoice')}}</a>
                                            </div>
                                        </div>
                                    </td>
                                </tr>
