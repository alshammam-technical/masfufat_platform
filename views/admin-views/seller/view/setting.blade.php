...
@section('content')
    <div class="content container-fluid">
        ...
        <div class="flex-between d-sm-flex row align-items-center justify-content-between mb-2 mx-1">
            <div>
                <a href="{{route('admin.sellers.seller-list')}}" class="btn btn--primary btn-primary mt-3 mb-3">{{\App\CPU\Helpers::translate('Back_to_seller_list')}}</a>
            </div>
            <div>
                @if ($seller->status=="pending")
                    <div class="mt-4 pr-2">
                        <div class="flex-start">
                            ...
                            <div><h4>{{\App\CPU\Helpers::translate('Seller request for open a shop')}}.</h4></div>
                        </div>
                        <div class="text-center">
                            <form class="d-inline-block" action="{{route('admin.sellers.updateStatus')}}" method="POST">
                                ...
                                <button type="submit" class="btn btn--primary btn-primary btn-sm">{{\App\CPU\Helpers::translate('Approve')}}</button>
                            </form>
                            <form class="d-inline-block" action="{{route('admin.sellers.updateStatus')}}" method="POST">
                                @csrf
                                <input type="hidden" name="id" value="{{$seller->id}}">
                                <input type="hidden" name="status" value="rejected">
                                <button type="submit" class="btn btn-danger btn-sm">{{\App\CPU\Helpers::translate('reject')}}</button>
                            </form>
                        </div>
                    </div>
                @endif
        ...
        <div class="page-header">
            ...
            <div class="js-nav-scroller hs-nav-scroller-horizontal">
                ...
                <ul class="nav nav-tabs lightSliderrflex-wrap page-header-tabs">
                    <li class="nav-item">
                        <a class="nav-link " href="{{ route('admin.sellers.view',$seller->id) }}">{{\App\CPU\Helpers::translate('seller primary data')}}</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link "
        ...
        <div class="row">
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            <small class="badge badge-soft-info text-wrap mb-3">
                                {{\App\CPU\Helpers::translate('If sales commission is disabled here, the system default commission will be applied')}}.
                            </small>
                            <div class="form-group">
                                <label>{{\App\CPU\Helpers::translate('Sales Commission')}} ( % )</label>
            ...
            <div class="col-md-6 mt-3">
                <form action="{{ url()->current() }}"
                    ...
                    <div class="card">
                        <div class="card-header">
                            <h5 class="mb-0"> {{\App\CPU\Helpers::translate('GST Number')}} : </h5>
                            <label class="switcher">
                        ...
                        <div class="card-body">
                            <small class="badge text-wrap badge-soft-info mb-3">
                                {{\App\CPU\Helpers::translate('If GST number is disabled here, it will not show in invoice')}}.
                            </small>
                            <div class="form-group">
                                <label> {{\App\CPU\Helpers::translate('GST Number')}}  </label>
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <form action="{{ url()->current() }}"
                              method="GET">
                            @csrf
                            <label class="title-color d-flex">{{\App\CPU\Helpers::translate('Seller POS permission on/off')}}</label>
