    ...
    <div class="content container-fluid">
        ...
        <div class="mb-3">
            <h2 class="h1 mb-0 text-capitalize d-flex gap-2 align-items-center">
                ...
                {{\App\CPU\Helpers::translate('Seller_details')}}
            </h2>
        </div>
        <!-- End Page Title -->
                ...
                @if ($seller->status=="pending")
                    <div class="mt-4 pr-2">
                        <div class="flex-start">
                            ...
                            <div>{{\App\CPU\Helpers::translate('Seller_request_for_open_a_shop.')}}</div>
                        </div>
                        ...
                        <div class="px-3 py-4">
                            <div class="row align-items-center">
                                <div class="col-lg-4 mb-3 mb-lg-0">
                                    <h5 class="mb-0 text-capitalize d-flex gap-1 align-items-center">{{ \App\CPU\Helpers::translate('transaction_table')}}
                                    ...
                                    <form action="{{ url()->current() }}" method="GET">
                                        ...
                                        <div class="input-group input-group-merge input-group-custom">
                                            ...
                                            <input id="datatableSearch_" type="search" name="search"
                                                   class="form-control"
                                                   placeholder="{{\App\CPU\Helpers::translate('Search by orders id or transaction id')}}"
                                        ...
                                        <div class="d-flex justify-content-end align-items-center gap-10">
                                            <select class="form-control" name="status">
                                                <option value="0" selected disabled>
                                                    ---{{\App\CPU\Helpers::translate('select_status')}}---
                                                </option>
                                                <option class="text-capitalize"
                                                        value="all" {{ $status == 'all'? 'selected' : '' }} >{{\App\CPU\Helpers::translate('all')}} </option>
                                                <option class="text-capitalize"
                                                        value="disburse" {{ $status == 'disburse'? 'selected' : '' }} >{{\App\CPU\Helpers::translate('disburse')}} </option>
                                                <option class="text-capitalize"
                                                        value="hold" {{ $status == 'hold'? 'selected' : '' }}>{{\App\CPU\Helpers::translate('pending_')}}</option>
                                            </select>
                    ...
                    <div class="table-responsive">
                        <table id="datatable"
                            ...
                            <tr>
                                ...
                                <th>{{\App\CPU\Helpers::translate('seller_name')}}</th>
                                <th>{{\App\CPU\Helpers::translate('customer_name')}}</th>
                                <th>{{\App\CPU\Helpers::translate('order_id')}}</th>
                                <th>{{\App\CPU\Helpers::translate('transaction_id')}}</th>
                                <th>{{\App\CPU\Helpers::translate('order_amount')}}</th>
                                <th>{{ \App\CPU\Helpers::translate('seller_amount') }}</th>
                                <th>{{\App\CPU\Helpers::translate('admin_commission')}}</th>
                                <th>{{\App\CPU\Helpers::translate('received_by')}}</th>
                                <th>{{\App\CPU\Helpers::translate('delivered_by')}}</th>
                                <th>{{\App\CPU\Helpers::translate('delivery_charge')}}</th>
                                <th>{{\App\CPU\Helpers::translate('payment_method')}}</th>
                            ...
                            @foreach($transactions as $key=>$transaction)
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        @else
                                            {{ App\Model\Seller::find($transaction['seller_id'])->company_name }}
                                        @endif
                                    </td>
                                    <td>
                                        {{ App\User::find($transaction['customer_id'])->name??'' }}
                                    </td>
                                    <td>{{$transaction['order_id']}}</td>
                                    <td>{{$transaction['transaction_id']}}</td>
                                    <td>{{\App\CPU\BackEndHelper::set_symbol(($transaction['order_amount']))}}</td>
                                    <td>{{\App\CPU\BackEndHelper::set_symbol(($transaction['seller_amount']))}}</td>
                                    <td>{{\App\CPU\BackEndHelper::set_symbol(($transaction['admin_commission']))}}</td>
                                    <td>{{$transaction['received_by']}}</td>
                                    <td>{{$transaction['delivered_by']}}</td>
                                    <td>{{\App\CPU\BackEndHelper::set_symbol(($transaction['delivery_charge']))}}</td>
                                    <td>{{str_replace('_',' ',$transaction['payment_method'])}}</td>
                                    <td>{{\App\CPU\BackEndHelper::set_symbol(($transaction['tax']))}}</td>
                                    <td class="text-center">
                                        @if($transaction['status'] == 'disburse')
                                            <span class="badge badge-soft-success">
                                                {{\App\CPU\Helpers::translate('disburse')}}
                                            </span>
                                        @else
                                            <span class="badge badge-soft-warning ">
                                                {{\App\CPU\Helpers::translate('pending_')}}
                                            </span>
                                        @endif
