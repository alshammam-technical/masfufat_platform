...
@section('title', \App\CPU\Helpers::translate('Package List'))
@push('css_or_js')
@endpush
        ...
        <div class="row" style="margin-top: 20px">
            <div class="col-lg-5">
                <nav aria-label="breadcrumb" style="width:100%; text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item" aria-current="page">{{\App\CPU\Helpers::translate('Packages')}}</li>
                    </ol>
                </nav>
            </div>
            ...
            <div class="col-lg-7" hidden>
                <div class="d-flex table-actions flex-wrap justify-content-end mx-3">
                    <div class="d-flex">
                    <a title="{{Helpers::translate('Add new')}}" class="btn ti-plus btn-success my-2 btn-icon-text m-2" href="{{route('admin.ServicesPackaging.add-new')}}">
                        ...
                        <div class="dropdown dropdown">
                            ...
                            <div aria-labelledby="droprightMenuButton" class="dropdown-menu">
                                ...
                                <div aria-labelledby="droprightMenuButton" class="dropdown-menu tx-13" style="position: absolute;will-change: transform;top: -10%;left: -100%;transform: translate3d(0px, 145px, 0px);">
                                    <a class="dropdown-item bulk-export" href="{{route('admin.ServicesPackaging.bulk-export')}}">
                                        {{\App\CPU\Helpers::translate('export to excel')}}
                                    </a>
                                    <a class="dropdown-item bulk-import" href="{{route('admin.ServicesPackaging.bulk-import')}}">
                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <table style="text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};"
                                ...
                                @foreach($br as $k=>$b)
                                    <tr>
                                        <form class="table-editor-form" method="POST" action="{{route('admin.ServicesPackaging.update',[$b['id']])}}">
                                            <td>
                                                @csrf
                                            ...
                                            @foreach(\App\CPU\Helpers::get_langs() as $lang)
                                            <td>
                                                <span class="spanValue" style="white-space: initial">
                                                    {{ \App\CPU\Helpers::get_prop('App\services_packaging',$b['id'],'name',$lang) ?? $name }}
                                                </span>
                                                <input class="form-control editValue lang_value" type="text"
                                                value="{{ \App\CPU\Helpers::get_prop('App\services_packaging',$b['id'],'name',$lang) ?? $name }}"
                                                name="name[]"
                                            ...
                                            <td>
                                                ...
                                                href="{{route('admin.ServicesPackaging.update',[$b['id']])}}">
                                                    <i class="tio-edit"></i>
                                                </a>
                                            </td>
                                        </form>
                        ...
                        <form hidden action="{{route('admin.ServicesPackaging.bulk-delete')}}" method="post" id="bulk-delete">
                            @csrf @method('delete')
                            <input type="text" name="ids" class="ids">
                            <input type="text" name="not_ids" class="not_ids">
                        </form>

                        <form hidden action="{{route('admin.ServicesPackaging.bulk-status',['status'=>true])}}" method="post" id="bulk-enable">
                            @csrf @method('post')
                            <input type="text" name="ids" class="ids">
                            <input type="text" name="not_ids" class="not_ids">
                        </form>

                        <form hidden action="{{route('admin.ServicesPackaging.bulk-status',['status'=>false])}}" method="post" id="bulk-disable">
                            @csrf @method('post')
                            <input type="text" name="ids" class="ids">
                            <input type="text" name="not_ids" class="not_ids">
                        </form>
