    ...
    <div class="content container-fluid ">
        ...
        <div class="mb-4">
            <h2 class="h1 mb-0 text-capitalize d-flex align-items-center gap-2">
                ...
                {{\Helpers::translate('transaction_table')}}
                        ...
                        <form action="" method="GET">
                            ...
                            <div class="input-group input-group-merge input-group-custom">
                                ...
                                <input id="datatableSearch_" type="search" name="search" class="form-control"
                                       placeholder="{{ \Helpers::translate('Search by orders id or transaction id')}}"
                                    ...
                                    <div class="">
                                        <select class="js-select2-custom form-control" name="customer_id">
                                            <option class="text-center" value="0">
                                                ---{{\Helpers::translate('select_customer')}}---
                                            </option>
                                    ...
                                    <div class="">
                                        <select class="form-control" name="status">
                                            ...
                                            <option class="text-capitalize"
                                                    value="all" {{ $status == 'all'? 'selected' : '' }} >{{\Helpers::translate('all')}} </option>
                                            <option class="text-capitalize"
                                                    value="disburse" {{ $status == 'disburse'? 'selected' : '' }} >{{\Helpers::translate('disburse')}} </option>
                                            <option class="text-capitalize"
                                                    value="hold" {{ $status == 'hold'? 'selected' : '' }}>{{\Helpers::translate('hold')}}</option>
                                        </select>
                                    </div>
                                </div>
                                <div class="col-md-3">
                                    ...
                                    <div>
                                        ...
                                        <ul class="dropdown-menu dropdown-menu-right">
                                            <li><a class="dropdown-item"
                                                   href="{{ route('admin.transaction.transaction-export', ['customer_id'=>request('customer_id'), 'status'=>request('status'), 'from'=>request('from'), 'to'=>request('to')]) }}"  >{{\Helpers::translate('Excel')}}</a>
                                            </li>
            ...
            <div class="table-responsive">
                <table id="datatable"
                    ...
                    <tr>
                        ...
                        <th>{{\Helpers::translate('seller_name')}}</th>
                        <th>{{\Helpers::translate('customer_name')}}</th>
                        <th>{{\Helpers::translate('order_id')}}</th>
                        <th>{{\Helpers::translate('transaction_id')}}</th>
                        <th>{{\Helpers::translate('order_amount')}}</th>
                        <th>{{ \Helpers::translate('seller_amount') }}</th>
                        <th>{{\Helpers::translate('admin_commission')}}</th>
                        <th>{{\Helpers::translate('received_by')}}</th>
                        <th>{{\Helpers::translate('delivered_by')}}</th>
                        <th>{{\Helpers::translate('delivery_charge')}}</th>
                        <th>{{\Helpers::translate('payment_method')}}</th>
                        <th>{{\Helpers::translate('tax')}}</th>
                        <th>{{\Helpers::translate('date')}}</th>
                        <th>{{\Helpers::translate('status')}}</th>
                    </tr>
                    </thead>
                    <tbody>
        ...
        $('#from_date,#to_date').change(function () {
            ...
            if (fr != '' && to != '') {
                if (fr > to) {
                    $('#from_date').val('');
                    $('#to_date').val('');
                    toastr.error('{{\Helpers::translate('Invalid date range')}}!', Error, {
                        CloseButton: true,
                        ProgressBar: true
                    });
                }
