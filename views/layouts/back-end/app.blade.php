...
<head>
    ...
    <link rel="stylesheet" href="{{asset('public/assets/back-end')}}/css/custom.css?v=4">
    ...
    <link rel="stylesheet" href="{{asset('public/assets/back-end')}}/css/style.css?v=3">
    ...
    <style>
        @isset($disableDatatableColsWidth)
        @else
        .datatable * td:not(:first-of-type,.id_col), .datatable * th:not(:first-of-type,.id_col), .products-dataTable * td:not(:first-of-type,.id_col), .products-dataTable * th:not(:first-of-type,.id_col){
...
@endif
<!-- END ONLY DEV -->
<main id="content" role="main" class="main pointer-event {{isset($hide_all) ? 'pt-0' : ''}}">
    <!-- Content -->
@yield('content')
<!-- End Content -->
...
<script>
    ...
    $(document).on('ready', function () {
        // Loop through all input fields of type "number" on document ready
...
<script>
    ...
    var table = $('.datatable').DataTable({
        ...
        initComplete: function () {
            ...
            this.api().columns().every( function (e) {
                var column = this;
                var namee = $(".theadF tr .theadFilter").eq(column.index()).attr('field') ?? null;
                var select = $('<input type="text" class="form-control dt-filter" placeholder="" name="'+namee+'" />')
                    .appendTo( $(".theadF tr .theadFilter").eq(column.index()) )
                    .on( 'change keyup', function (e){
                        if ($('.datatable').hasClass('r-r')) {
                            $(".filters-form").find("input[name='"+namee+"']").remove();
                            var a = $(this).clone().appendTo(".filters-form");
                            a.attr('type','hidden');
                            if(e.keyCode == 13){
    ...
    $(document).ready(function () {
        ...
        @if(request()->route()->getPrefix() == "admin/product")
            @if (\Request::route()->getName() == 'admin.product.updated-product-list')
            @php($prfx = request()->route()->getPrefix().'s.updated-product-list')

            @else
                @if(\Request::exists('status'))
                @php($prfx = request()->route()->getPrefix().'s.'.request('type').'.'.request('status'))
                @else
                @php($prfx = request()->route()->getPrefix().'s.'.request('type'))
                @endif
            @endif
        @elseif(request()->route()->getPrefix() == "admin/orders")
            @php($prfx = 'order'.'.'.request("status"))
        @elseif(request()->route()->getPrefix() == "admin/abandoned-carts")
            @php($prfx = str_replace('admin','marketing',request()->route()->getPrefix().''))
            @php($prfx = str_replace('abandoned-carts','abandoned_carts',$prfx))
        @elseif(request()->route()->getPrefix() == "admin/affiliate")
            @php($prfx = str_replace('admin','marketing',request()->route()->getPrefix().''))
        @elseif(request()->route()->getPrefix() == "admin/stores")
            @php($prfx = str_replace('stores','customers',request()->route()->getPrefix().''))
        @elseif(request()->route()->getPrefix() == "admin/customer")
            @php($prfx = str_replace('customer','end_user',request()->route()->getPrefix().''))
        @elseif(in_array(request()->route()->getName(),['admin.business-settings.terms-condition','admin.business-settings.privacy-policy','admin.business-settings.warranty-policy','admin.business-settings.about-us']))
            @php($prfx = str_replace('','',request()->route()->getName()))
        @else
        @php($prfx = request()->route()->getPrefix().'')
        @endif
        @if(!Helpers::module_permission_check(str_replace('/','.',$prfx).'.add'))
        $(".btn.ti-plus:not(.ui-sortable-handle)").remove();
        @endif
        @if(!Helpers::module_permission_check(str_replace('/','.',$prfx).'.edit'))
            @if(!Helpers::module_permission_check(str_replace('/','.',$prfx).''))
            $(".edit-btn:not(.ui-sortable-handle)").remove();
            $(".save-btn:not(.ui-sortable-handle)").remove();
            @endif
        @endif
        @if(request('status') !== "all" && !Helpers::module_permission_check(str_replace('/','.',$prfx).'.order_status'))
        $(".order_status:not(.ui-sortable-handle)").remove();
        @endif
        @if(request('status') !== "all" && !Helpers::module_permission_check(str_replace('/','.',$prfx).'.shipping_status'))
        $(".shipping_status:not(.ui-sortable-handle)").remove();
        @endif
        @if(!Helpers::module_permission_check(str_replace('/','.',$prfx).'.add_from'))
        $(".btnAddFrom:not(.ui-sortable-handle)").remove();
        @endif
        @if(!Helpers::module_permission_check(str_replace('/','.',$prfx).'.delete'))
        $(".btnDeleteRow:not(.ui-sortable-handle)").remove();
        @endif
        @if(!Helpers::module_permission_check(str_replace('/','.',$prfx).'.show_for_pricing_levels'))
        $(".bulk-pricing-form:not(.ui-sortable-handle)").remove();
        @endif
        @if(!Helpers::module_permission_check(str_replace('/','.',$prfx).'.show_product'))
        $(".bulk-show-product:not(.ui-sortable-handle)").remove();
        @endif
        @if(!Helpers::module_permission_check(str_replace('/','.',$prfx).'.import'))
        $(".bulk-import:not(.ui-sortable-handle)").remove();
        @endif
        @if(!Helpers::module_permission_check(str_replace('/','.',$prfx).'.export'))
        $(".bulk-export:not(.ui-sortable-handle)").remove();
        @endif
        @if(!Helpers::module_permission_check(str_replace('/','.',$prfx).'.export') && !Helpers::module_permission_check(str_replace('/','.',$prfx).'.import'))
        $(".bulk-import-export:not(.ui-sortable-handle)").remove();
        @endif
        @if(!Helpers::module_permission_check(str_replace('/','.',$prfx).'.enable'))
        $(".ti-check").closest('.dropdown-item').remove();
        @endif
        @if(!Helpers::module_permission_check(str_replace('/','.',$prfx).'.disable'))
        $(".ti-close").closest('.dropdown-item').remove();
        @endif
        @if(!Helpers::module_permission_check(str_replace('/','.',$prfx).'.show_product'))
        $(".bulk-show-product:not(.ui-sortable-handle)").remove();
        @endif
        @if(!Helpers::module_permission_check(str_replace('/','.',$prfx).'.bulk_edit'))
        $(".bulk_edit:not(.ui-sortable-handle)").remove();
        @endif
        @if(request('status') !== "all" && !Helpers::module_permission_check(str_replace('/','.',$prfx).'.generate_bill_of_lading'))
        $(".generate_bill_of_lading:not(.ui-sortable-handle)").remove();
        @endif
        @if(request('status') !== "all" && !Helpers::module_permission_check(str_replace('/','.',$prfx).'.print_bill_of_lading'))
        $(".print_bill_of_lading:not(.ui-sortable-handle)").remove();
        @endif
        @if(request('status') !== "all" && !Helpers::module_permission_check(str_replace('/','.',$prfx).'.generate_invoice'))
        $(".generate_invoice:not(.ui-sortable-handle)").remove();
        @endif

        $(document).on("change",".table * input[type='file']",function(){
            var input = this;
        ...
        $(document).on("change",".SumoSelect-custom,.SumoSelect-noImg,.multiselect,.testselect2-custom",function(){
            if($(this).val()){
    ...
    $(document).on('click','.nav-item',function(){
        ...
        $(this).closest('.nav-tabs').next('.tab-content').find(".tab-pane").removeClass("show active");
        $(this).closest('.nav-tabs').next('.tab-content').find(".tab-pane[aria-labelledby="+id+"]").addClass("show active");
    })
    ...
    function translate(word){
        $.ajax({
            url:"{{ route('admin.translate-now') }}?word="+word,
            success:function(data){
                return wrod;
            }
        })
        ...
        @foreach (\App\CPU\Helpers::get_langs() as $key=>$lang)
            $(e.target).closest(parent).find(selector+":eq({{$key}})").each(function(){
                ...
                if(!$(this).val()){
                    $.ajax({
                        type:'get',
                        url:"{{route('home')}}/admin/g-translate/"+e.target.value+"/{{$lang == 'sa' ? 'ar' : $lang}}",
                        success:function(data){
                            ths.val(data);
                        }
                    })
...
<script>
    ...
    function addFrom(table){
        var id = $(".ids").val()
        if(id){
            location.replace("{{route('admin.clone')}}?table="+table+"&id="+id);
        }
    }
</script>
@stack('script')
...
<script>
    ...
    function update_lang(key, value) {
        ...
        $.ajax({
            url: "{{route('admin.business-settings.language.translate-submit',[session()->get('local') ?? 'sa'])}}",
            method: 'POST',
            data: {
                key: key,
                value: value
                ...
                data: [
                    ...
                    },
                    {
                        ...
                        action: function(e, selector) {
                            ...
                            $.ajax({
                                url:"{{ route('admin.translate-now') }}?word="+title_,
                                success:function(data){
