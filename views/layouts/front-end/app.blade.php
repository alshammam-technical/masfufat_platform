...
<body class="toolbar-enabled navbar-vertical-aside-closed-mode mb-0">
    ...
    @if (isset($announcement) && $announcement['status']==1)
        <div class="d-flex justify-content-between align-items-center anouncementDiv" id="anouncement" style="background-color: #fdcd05;color:#000000;top: 0;position: fixed;z-index: 1034;">
            <span></span>
    ...
    <div id="main">
        ...
        <!-- Page Content-->
        <div @isset($hide_all) @else id="content" @endisset>
            @yield('content')
        </div>
...
<script>
    ...
    function addAllToLinked(e,el){
        var products_id = $("#linkedProducts").val();
        $.ajax({
            url:"{{ route('linked-products.add') }}?product_id="+products_id,
            success:function(data){
                $(el).attr('disabled','disabled');
                $("#linkedProducts").val('');
                $(".product-single-hover.checked-product").each(function(){
                    ...
                    $(this).find(".btn-add-linked").hide();
                    $(this).find(".btn-add-linked").prev('.btn').removeClass('col-6');
                    $(this).find(".btn-add-linked").prev('.btn').addClass('col-12');
                })
