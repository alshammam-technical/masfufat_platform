    ...
    <div class="content container-fluid" style="padding-{{(Session::get('direction') == 'rtl') ? 'right' : 'left'}}: 17%">
        ...
        <div class="mb-3">
            <h2 class="h1 mb-0 text-capitalize d-flex align-items-center gap-2">
                ...
                {{\App\CPU\Helpers::translate('Withdraw_Request')}}
            </h2>
        </div>
        <!-- End Page Title -->
                        ...
                        <div class="row align-items-center">
                            ...
                            <div class="col-lg-8 mt-3 mt-lg-0 d-flex gap-3 justify-content-lg-end">
                                ...
                                <ul class="dropdown-menu dropdown-menu-right">
                                    <li><a class="dropdown-item" href="{{route('seller.delivery-man.withdraw-list-export')}}">Excel</a></li>
                                ...
                                <select name="delivery_withdraw_status_filter" onchange="status_filter(this.value)" class="custom-select min-w-120 max-w-200">
                                    ...
                                    <option value="approved" {{session()->has('delivery_withdraw_status_filter') && session('delivery_withdraw_status_filter') == 'approved'?'selected':''}}>{{\App\CPU\Helpers::translate('Approved')}}</option>
                                    <option value="denied" {{session()->has('delivery_withdraw_status_filter') && session('delivery_withdraw_status_filter') == 'denied'?'selected':''}}>{{\App\CPU\Helpers::translate('Denied')}}</option>
                                    <option value="pending" {{session()->has('delivery_withdraw_status_filter') && session('delivery_withdraw_status_filter') == 'pending'?'selected':''}}>{{\App\CPU\Helpers::translate('Pending')}}</option>
                                </select>
                    ...
                    <div class="table-responsive">
                        <table id="datatable"
                            ...
                            <tr>
                                ...
                                <th>{{\App\CPU\Helpers::translate('amount')}}</th>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="d-flex justify-content-center">
                                            @if (isset($wr->delivery_men))
                                            <a href="{{route('seller.delivery-man.withdraw-view',[$wr['id']])}}"
...
@push('script_2')
  <script>
      function status_filter(type) {
          ...
          $.post({
              url: '{{route('seller.delivery-man.status-filter')}}',
              data: {
