...
<div class="card-header">
    <h4 class="d-flex align-items-center text-capitalize gap-10 mb-0">
        ...
        {{\App\CPU\Helpers::translate('most_popular_products')}}
    </h4>
</div>
<!-- End Header -->
                    ...
                    @foreach($most_rated_products as $key=>$item)
                        ...
                        @if(isset($product))
                            <div class="cursor-pointer grid-card" onclick="location.href='{{route('seller.product.view',[$item['product_id']])}}'">
                                <div class="">
                                    <img class="avatar avatar-bordered border-gold avatar-60 rounded" src="{{asset("storage/app/public/product/$current_lang")}}/{{(isset(json_decode($product['images'])->$current_lang)) ? json_decode($product['images'])->$current_lang[0] ?? '' : ''}}"
