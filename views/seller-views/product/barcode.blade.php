        ...
        <div class="col-sm-12 col-lg-12 mb-3 mb-lg-2">
            ...
            <div class="mb-3">
                <h2 class="h1 mb-0 text-capitalize d-flex gap-2">
                    ...
                    {{\App\CPU\Helpers::translate('generate_barcode')}}
                </h2>
            </div>
            <!-- End Page Title -->
                    ...
                    <div class="table-responsive">
                        <table style="text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};"
                            ...
                            <thead class="thead-light thead-50 text-capitalize">
                                <tr>
                                    <th>{{ \App\CPU\Helpers::translate('code') }}</th>
                                    ...
                                    <form action="{{ url()->current() }}" method="GET">
                                        ...
                                        <th>
                                            ...
                                            @else
                                                ...
                                                <a class="title-color hover-c1" href="{{route('seller.product.edit',[$product['id']])}}">
                                                    {{ \App\CPU\Helpers::translate('update_your_product_code') }}
                                                </a>
                                        ...
                                        <th>
                                            <input id="limit" class="form-control" type="number" name="limit" min="1"
                                                value="{{ $limit }}">
                                            <span
                                                class="text-danger mt-1 d-block">{{ \App\CPU\Helpers::translate('maximum_quantity_270') }}</span>
                                        </th>

                                        <th>
                                            <div class="d-flex justify-content-center gap-2">
                                                <button class="btn btn-outline-info"
                                                    type="submit">{{ \App\CPU\Helpers::translate('generate_barcode') }}</button>
                                                <a href="{{ route('seller.product.barcode', [$product['id']]) }}"
                                                    class="btn btn-outline-danger">{{ \App\CPU\Helpers::translate('reset') }}</a>
                                                <button type="button" id="print_bar" onclick="printDiv('printarea')"
                                                    class="btn btn-outline--primary ">{{ \App\CPU\Helpers::translate('print') }}</button>
                                            </div>
                                        </th>
                                    </form>
                                </tr>
    ...
    <div class="row">
        <div class="col-12 mt-5 p-4">
            <h1 class="style-one-br show-div2">
                {{ \App\CPU\Helpers::translate("This page is for A4 size page printer, so it won't be visible in smaller devices") }}.
            </h1>
        </div>
    </div>
    ...
    <div id="printarea" class="show-div pb-5">
        ...
        <div class="item style24">
            ...
            <div class="barcode_price text-capitalize">
                {{ \App\CPU\BackEndHelper::set_symbol(($product->unit_price)) }}
            </div>

            @if ($product->code !== null)
                <div class="barcode_image">{!! DNS1D::getBarcodeHTML($product->code, 'CODABAR') !!}</div>
                <div class="barcode_code text-capitalize">{{ \App\CPU\Helpers::translate('code') }} : {{ $product->code }}</div>
            @else
                <p class="text-danger">{{ \App\CPU\Helpers::translate('please_update_product_code') }}</p>
            @endif
        </div>
        @endfor
    </div>
...
@endsection
@push('script_2')
    <script src={{ asset('public/assets/seller/js/global.js') }}></script>
    <script>
