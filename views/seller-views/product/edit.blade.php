...
@push('css_or_js')
    ...
    <style>
        ...
        .optWrapper:not(.sumo_linked_products_ids .optWrapper){
...
@section('content')
    ...
    $seller_shipping = \App\Model\ShippingType::where('seller_id',0)->first();
    ...
    <div class="content container-fluid">
            ...
            <div class="col-lg-5">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item mx-1" aria-current="page"><a href="{{route('seller.product.list',['type'=>'in_house'])}}">{{\App\CPU\Helpers::translate('Products')}}</a></li>
        ...
        <div class="card">
            <div class="row">
                <div class="col-md-12">
                    <form class="product-form" action="{{($product->id && \Request::route()->getName() !== 'seller.clone') ? route('seller.product.update',$product->id) : route('seller.product.update',0)}}" method="post"
                                ...
                                <div class="row">
                                            ...
                                            @foreach(Helpers::get_langs() as $lang)
                                                ...
                                                <div class="{{$lang != session()->get('local')? 'd-none':''}} lang_form" id="{{$lang}}-form">
                                                    <div class="form-group">
                                                        <label class="title-color label-w-btn" for="{{$lang}}_name">{{\App\CPU\Helpers::translate('name')}}<span class="text-danger">*</span>
                                                            ({{strtoupper($lang)}})
                                                            <a class="btn btn-primary" onclick="emptyInput(event,'.card-body','.product-name')">{{\App\CPU\Helpers::translate('Field dump')}}</a>
                                                        </label>
                                                        <input type="text" {{$lang == 'en'? 'required':''}} name="name[]"
                                                            ...
                                                            class="form-control product-name" placeholder="{{\App\CPU\Helpers::translate('name')}}" required>
                                                    </div>

                                                    <div class="form-group">
                                                        <label class="d-flex title-color label-w-btn" for="{{$lang}}_short_desc">{{\App\CPU\Helpers::translate('Brief description')}}<span class="text-danger"></span>
                                                            ({{strtoupper($lang)}})
                                                            <label class="ckbox mx-2" style="margin-top: 2px">
                                                                <input value="1" name="short_desc_qv[]"
                                                                @if(Helpers::get_prop('App\Model\Product',$product->id,'short_desc_qv',"$lang")) checked @endif
                                                                type="checkbox"><span class="py-0 px-2 m-0"> {{ Helpers::translate('show in quick view') }} </span>
                                                            </label>
                                                            <a class="btn btn-primary" onclick="emptyInput(event,'.card-body','.product-short_desc')">{{\App\CPU\Helpers::translate('Field dump')}}</a>
                                                        </label>
                                                        <input type="text" {{$lang == 'en'? 'required':''}} name="short_desc[]"
                                                            onchange="translateName(event,'.card-body','input[name=\'short_desc[]\']')"
                                                            id="{{$lang}}_short_desc"
                                                            value="{{$translate[$lang]['short_desc']??$product['short_desc']}}"
                                                            class="form-control product-short_desc" placeholder="{{\App\CPU\Helpers::translate('Brief description')}}" required>
                                                    </div>
                                                    ...
                                                    <div class="form-group">
                                                        <label class="title-color label-w-btn" for="{{$lang}}_promo_title">{{\App\CPU\Helpers::translate('Promo title')}}<span class="text-danger"></span>
                                                            ({{strtoupper($lang)}})



                                                            <a class="btn btn-primary" onclick="emptyInput(event,'.card-body','.product-promo_title')">{{\App\CPU\Helpers::translate('Field dump')}}</a>
                                                        </label>
                                                        <input type="text" {{$lang == 'en'? 'required':''}} name="promo_title[]"
                                                            onchange="translateName(event,'.card-body','input[name=\'promo_title[]\']')"
                                                            id="{{$lang}}_promo_title"
                                                            value="{{$translate[$lang]['promo_title']??$product['promo_title']}}"
                                                            class="form-control product-promo_title" placeholder="{{\App\CPU\Helpers::translate('Promo title')}}" required>
                                                    </div>
                                                    ...
                                                    <div class="form-group pt-4 editor-textarea-container">
                                                        <label class="title-color label-w-btn">{{\App\CPU\Helpers::translate('description')}}
                                                            ({{strtoupper($lang)}})
                                                            <a class="btn btn-primary" onclick="emptyDesc()">{{\App\CPU\Helpers::translate('Field dump')}}</a>
                                                        </label>
                                                        <textarea name="description[]" class="textarea{{$lang}} editor-textarea" id="textarea{{$lang}}" onchange="translateDesc(event)"
                                    ...
                                    <div class="card col-lg-4 mt-2 px-3">
                                        <div class="card-body px-0">
                                            <div class="input-group mb-4 w-100 multiSelect2Div">
                                                ...
                                                <div class="form-control p-0">
                                                    <select
                                                        ...
                                                        multiple
                                                        id="category_id"
                                                        onchange="getRequest('{{url('/')}}/seller/product/get-categories?parent_id='+this.value,'sub-category-select','select');refreshMainCat(this)">
                                                        @foreach($categories as $category)
                                                            <option value="{{$category['id']}}" {{ (in_array($category->id,explode(',',(isset($product_category[0])) ? $product_category[0]->id : '[]'))) ? 'selected' : ''}}
                                                                icon='{{asset("storage/app/public/category/".(\App\CPU\Helpers::get_prop('App\Model\Category',$category['id'],'image',session()->get('local')) ?? Helpers::get_prop('App\Model\Category',$category['id'],'icon',session()->get('local')) ?? $category->icon))}}'
                                                                >
                                            ...
                                            <div class="input-group mb-3">
                                                <div class="input-group-text bg-primary text-light text-start">
                                                    <span class="" style="width: 276px" id="basic-addon1">{{\App\CPU\Helpers::translate('Item Number')}}</span>
                                                </div>
                                                <input name="item_number" aria-describedby="basic-addon1" value="{{$product->item_number}}" class="form-control text-dark" placeholder="" type="text" required>
                                            </div>

                                            <div class="input-group mb-3">
                                                <div class="input-group-text bg-primary text-light text-start">
                                                    <span class="" style="width: 276px" id="basic-addon1">{{\App\CPU\Helpers::translate('product_code_sku')}}
                                                        <a class="style-one-pro mx-2 px-1 py-0 btn btn-info text-dark" style="cursor: pointer;" onclick="document.getElementById('generate_number').value = getRndInteger()">{{ \App\CPU\Helpers::translate('generate') }}
                                                            {{ \App\CPU\Helpers::translate('code') }}
                                                        </a>
                                                    </span>
                                                </div>
                                            ...
                                            <div class="input-group mb-3">
                                                <div class="input-group-text bg-primary text-light text-start">
                                                    <span class="" style="width: 276px" id="basic-addon1">{{\App\CPU\Helpers::translate('gtin')}}</span>
                                                </div>
                                                <input name="gtin" aria-describedby="basic-addon1" value="{{$product->gtin}}" class="form-control text-dark" placeholder="" type="text" required>
                                            </div>

                                            <div class="input-group mb-3">
                                                <div class="input-group-text bg-primary text-light text-start">
                                                    <span class="" style="width: 276px" id="basic-addon1">{{\App\CPU\Helpers::translate('mpn')}}</span>
                                                </div>
                                                <input name="mpn" aria-describedby="basic-addon1" value="{{$product->mpn}}" class="form-control text-dark" placeholder="" type="text" required>
                                            </div>

                                            <div class="input-group mb-3">
                                                <div class="input-group-text bg-primary text-light text-start">
                                                    <span class="" style="width: 276px" id="basic-addon1">{{\App\CPU\Helpers::translate('HS Code')}}</span>
                                                </div>
                                                <input name="hs_code" aria-describedby="basic-addon1" value="{{$product->hs_code}}" class="form-control text-dark" placeholder="" type="text" required>
                                            </div>
                                            ...
                                            <div class="bg-light pr-2 pl-2 m-0 mt-1 pt-2 pb-4 rounded-15 w-100">
                                                ...
                                                <div class="row mb-4">
                                                    ...
                                                    <div class="col-lg-12 publish_on_app publish_on_app_on" @if(!$product['publish_on_app_time']) style="display:none" @endif>
                                                        <div class="input-group mt-0 mb-0">
                                                            <div class="input-group-text bg-primary text-light text-start">
                                                                <span class="input-group-append border-0 text-light">{{\App\CPU\Helpers::translate('Publish date & time')}}</span>
                                                            </div>
                                                            <input name="publish_on_app_date" class="form-control border-0 fc-datepicker col-4 p-0" placeholder="DD/MM/YYYY" type="date" value="{{$product->publish_on_app_date ?? Carbon\Carbon::now()->format('Y-m-d')}}">                                                                <input name="publish_on_app_time" class="form-control border-0 col-13" placeholder="HH:mm" type="time" value="{{$product->publish_on_app_time ?? Carbon\Carbon::now()->format('H:i')}}">
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="mb-3 d-flex align-items-center gap-2">
                                                ...
                                                <div class="col-lg-12 p-0" style="overflow-y: auto;height: 1070px;">
                                                    <div class="input-group mb-2  p-2">
                                                        <div class="w-50 mt-2 pb-4">
                                                            <label class="ckbox">
                                                                <input value="1" class="switch_inputs has_tax" name="has_tax" inputsClass="tax_input" @if ($product->has_tax) checked @endif type="checkbox"><span>{{\App\CPU\Helpers::translate('The product is subject to tax')}}</span></label>
                                                        </div>
                        ...
                        @include('seller-views.product.quantityAndStock')
                                                                    ...
                                                                    <div class="col-lg-3 card-draggable product_imgs bg-primary m-3" imgIndex="{{$item}}" style="border-radius: 11px;width: 300px;max-width: 300px;min-width: 300px;;height:250px">
                                                                        <div class="card bg-white h-100" style="position: relative">
                                                                            <div class="card-body text-center pt-0 px-0" style="min-height: 260px">
                                                                                <a href="{{route('seller.product.remove-image',['id'=>$product['id'],'name'=>$photo,'lang'=>$lang])}}"
                                                                        ...
                                                                        <div style="position: relative;border-radius: 11px" class="border border-primary border-thick">
                                                                            <label class="file_upload bg-primary" style="width: 100%; height: 260px;border-radius: 11px; cursor: pointer; text-align: center; overflow: hidden; padding: 5px; margin-top: 5px; margin-bottom : 5px; position : relative; display: flex; align-items: center; margin: auto; justify-content: center; flex-direction: column;">
                                                                                <div class="card-body bg-primary px-0 pt-0" style="min-height: 260px">
                                                                                    <a href="{{route('seller.product.remove-video',['id'=>$product['id'],'name'=>$video,'lang'=>$lang])}}"
                                            ...
                                            <div class="w-100 justify-content-center">
                                                <div class="w-100 wk-text-center prop-c">
                                                    <div class="option-container" id="productOptions">
                                                        ...
                                                        @include('seller-views.product.products-option',['data'=>$item,'key'=>$key,'language'=>$language,'product'=>$product])
                                                        @php($key = $key + 1)
                                                        @endforeach
                                                    </div>
                                                    ...
                                                    <div class="option-values">
                                                        ...
                                                        {{--  @foreach (\App\CPU\Helpers::getSubProducts($product->id) ?? [] as $key=>$item)
                                                            @include('seller-views.product.option-value-acc',["subProdItem"=>$item,'values_sections'=>$key])
                                                        @endforeach  --}}
                                                    </div>
                                                </div>
                                            </div>
                            ...
                            <div class="card custom-card">
                                ...
                                >
                                    ...
                                    <div id="productProps">
                                        ...
                                        @if($key !== "is_shareable" && $key !== "show_product" && $key !== "selected_countries_show_quantity_number" && $key !== "countries" && $key !== "areas" && $key !== "cities" && $key !== "provinces")
                                            @include('seller-views.product.props',['propIndex'=>$key,'item'=>$item])
                                        @endif
                                        @endforeach
                                    </div>
                                    <div class="w-100 m-4">
                                ...
                                <div class="d-flex justify-content-end pt-0" hidden>
                                    @if($product->request_status == 2)
                                        <button type="button" hidden onclick="check()" class="btn btn--primary btn-primary btn-save">{{\App\CPU\Helpers::translate('Update & Publish')}}</button>
                                    @else
...
@push('script_2')
    ...
    <script>
        function viewValuesForOptions(){
            ...
            $.ajax({
                url:"{{route('seller.product.options.values.add-new.acc')}}?values_sections="+$(".option-value-collapse").length,
                type:"POST",
                processData: false,
                contentType: false,
                data:formData,
        ...
        function newProp(){
            $.ajax({
                url:"{{route('seller.product.props.add-new')}}?key="+$(".prop-c-c").length,
                success:function(data){
        ...
        function newOption(){
            ...
            $.ajax({
                url:"{{route('seller.product.options.add-new')}}?key="+l,
                success:function(data){
        ...
        function newValue(optionIndex){
            ...
            $.ajax({
                url:"{{route('seller.product.options.values.add-new')}}",
                data:{
    ...
    <script>
        ...
        $(document).ready(function(){
            ...
            $(document).on("click","#addNewPropBtn",function(e){
                ...
                $.ajax({
                    ...
                    url:"{{route('seller.productsProps.add-new')}}",
                    success:function(data){
        ...
        function deletePP(e,id){
            ...
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        url:"{{route('seller.productsProps.delete')}}",
                        type:"post",
                        data:{
                            _token: '{{csrf_token()}}',
                            id:id,
    ...
    <script>
        ...
        CKEDITOR.instances['textarea{{$lang}}'].on("blur", function(e) {
            ...
            if(val){
                ...
                if(!CKEDITOR.instances['textarea{{$lang}}'].getData()){
                    $.ajax({
                        type:'post',
                        url:"{{route('home')}}/seller/g-translate/{{$lang == 'sa' ? 'ar' : $lang}}",
                        data: {
                            _token: '{{ csrf_token() }}',
                            word: val,
                        },
        ...
        function refreshMainCat(ths){
            ...
            $('#main_category').find('option:not([disabled]):first').attr('selected','selected');
            $('#main_category').SumoSelect().sumo.reload()
        }
    </script>
    {{--ck editor--}}
