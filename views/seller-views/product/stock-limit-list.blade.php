    ...
    <div class="content container-fluid">
        ...
        <div class="mb-3 d-flex flex-column gap-1">
            <h2 class="h1 text-capitalize d-flex gap-2">
                ...
                {{\App\CPU\Helpers::translate('Limited_Stocked_Products_List')}}
                <span class="badge badge-soft-dark radius-50 fz-14 ml-1">{{ $pro->total() }}</span>
            </h2>
            <p class="d-flex">{{ \App\CPU\Helpers::translate('the_products_are_shown_in_this_list,_which_quantity_is_below') }} {{ $stock_limit }}</p>
        </div>
        <!-- End Page Title -->
                        ...
                        <div class="row justify-content-between align-items-center gy-2">
                                ...
                                <form action="{{ url()->current() }}" method="GET">
                                    <div class="input-group input-group-custom input-group-merge">
                                        ...
                                        <input id="datatableSearch_" type="search" name="search" class="form-control"
                                               placeholder="{{\App\CPU\Helpers::translate('Search Product Name')}}"
                            ...
                            <div class="col-12 mt-1 col-md-6 col-lg-3">
                                <select name="qty_ordr_sort" class="form-control"
                                        onchange="location.href='{{route('seller.product.stock-limit-list',['in_house', ''])}}/?sort_oqrderQty='+this.value">
                                    <option
                                        value="default" {{ $sort_oqrderQty== "default"?'selected':''}}>{{\App\CPU\Helpers::translate('default_sort')}}</option>
                                    <option
                                        value="quantity_asc" {{ $sort_oqrderQty== "quantity_asc"?'selected':''}}>{{\App\CPU\Helpers::translate('quantity_sort_by_(low_to_high)')}}</option>
                                    <option
                                        value="quantity_desc" {{ $sort_oqrderQty== "quantity_desc"?'selected':''}}>{{\App\CPU\Helpers::translate('quantity_sort_by_(high_to_low)')}}</option>
                                    <option
                                        value="order_asc" {{ $sort_oqrderQty== "order_asc"?'selected':''}}>{{\App\CPU\Helpers::translate('order_sort_by_(low_to_high)')}}</option>
                                    <option
                                        value="order_desc" {{ $sort_oqrderQty== "order_desc"?'selected':''}}>{{\App\CPU\Helpers::translate('order_sort_by_(high_to_low)')}}</option>
                                </select>
                            </div>
                    ...
                    <div class="table-responsive">
                        <table id="datatable"
                            ...
                            <tr>
                                ...
                                <th>{{\App\CPU\Helpers::translate('Product_Name')}}</th>
                                ...
                                <th class="text-center">{{\App\CPU\Helpers::translate('Active status')}}</th>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <a href="{{route('seller.product.view',[$p['id']])}}" class="media align-items-center gap-2">
                                            <img src="{{asset("storage/app/public/product/$current_lang")}}/{{(isset(json_decode($product['images'])->$current_lang)) ? json_decode($product['images'])->$current_lang[0] ?? '' : ''}}"
                                    ...
                                    <td>
                                        <div class="d-flex align-items-center product-quantity">
                                            ...
                                            <button class="btn py-0 px-2 fz-18" id="{{ $p->id }}"
                                                    ...
                                                    title="{{ \App\CPU\Helpers::translate('update_quantity') }}">
                                    ...
                                    <td>
                                        <div class="d-flex justify-content-center gap-2">
                                            <a class="btn btn-outline-info btn-sm square-btn" title="{{ \App\CPU\Helpers::translate('barcode') }}"
                    ...
                    <div class="modal-body">
                        ...
                        <div class="d-flex justify-content-end gap-10 flex-wrap align-items-center">
                            <button type="button" class="btn btn-danger px-4" data-dismiss="modal" aria-label="Close">
                                {{\App\CPU\Helpers::translate('close')}}
                            </button>
                            <button class="btn btn--primary btn-primary" class="btn btn--primary btn-primary px-4" type="submit">{{\App\CPU\Helpers::translate('submit')}}</button>
                        </div>
                    </div>
                </form>
            </div>
