...
<div class="content container-fluid">
    ...
    <div class="row gy-2">
            ...
            <div class="card">
                <div class="card-header">
                    <div class="row gy-1 justify-content-between align-items-center flex-grow-1">
                        ...
                        <h5 class="col-md-4 mb-0 text-capitalize">{{\App\CPU\Helpers::translate('refund_status')}}:
                ...
                <div class="card-body">
                    <div class="row gy-2">
                        ...
                        <div class="col-sm-8 col-md-4 col-lg-6">
                            ...
                            <div >{{\App\CPU\Helpers::translate('QTY')}} : {{$refund->order_details->qty}}</div>
                            <div>{{\App\CPU\Helpers::translate('price')}} : {{\App\CPU\Helpers::currency_converter($refund->order_details->price)}}</div>
                            @if ($refund->order_details->variant)
                                <strong><u>{{\App\CPU\Helpers::translate('Variation')}} : </u></strong>
                    ...
                    <div class="d-flex flex-wrap flex-column flex-md-row gap-10 justify-content-between">
                        <span class="title-color">{{\App\CPU\Helpers::translate('subtotal')}} : {{\App\CPU\Helpers::currency_converter($subtotal)}}</span>
                        <span class="title-color">{{\App\CPU\Helpers::translate('coupon_discount')}} : {{\App\CPU\Helpers::currency_converter($coupon_discount)}}</span>
                        <span class="title-color">{{\App\CPU\Helpers::translate('total_refund_amount')}} : {{\App\CPU\Helpers::currency_converter($refund_amount)}}</span>
                    </div>
                </div>
            </div>
        </div>
        <div class="col-12 ">
            <div class="card">
                <div class="card-header">
                    <h4 class="mb-0">{{\App\CPU\Helpers::translate('additional_information')}}</h4>
                </div>
                ...
                <div class="card-body">
                    <div class="row gy-2">
                        <div class="col-sm-6 col-md-4 d-flex flex-column gap-10">
                            ...
                            <span>{{\App\CPU\Helpers::translate('deliveryman_name')}} : {{$order->delivery_man!=null?$order->delivery_man->f_name . ' ' .$order->delivery_man->l_name:\App\CPU\Helpers::translate('not_assigned')}}</span>
                            <span>{{\App\CPU\Helpers::translate('deliveryman_email')}} : {{$order->delivery_man!=null?$order->delivery_man->email :\App\CPU\Helpers::translate('not_found')}}</span>
                            <span>{{\App\CPU\Helpers::translate('deliveryman_phone')}} : {{$order->delivery_man!=null?$order->delivery_man->phone :\App\CPU\Helpers::translate('not_found')}}</span>
                        </div>

                        <div class="col-sm-6 col-md-4 d-flex flex-column gap-10">
                            <span>{{\App\CPU\Helpers::translate('payment_method')}} : {{str_replace('_',' ',$order->payment_method)}}</span>
                            <span>{{\App\CPU\Helpers::translate('order_details')}} : <a class="btn btn--primary btn-primary btn-sm" href="{{route('seller.orders.details',['id'=>$order->id])}}">{{\App\CPU\Helpers::translate('click_here')}}</a></span>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="table-responsive datatable-custom">
                    <table class="table table-hover table-borderless table-thead-bordered table-nowrap table-align-middle card-table"
                        ...
                        <tr>
                            ...
                            <th>{{\App\CPU\Helpers::translate('note')}}</th>
                        </tr>
                        </thead>
