            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header flex-wrap gap-2">
                        <h5 class="mb-0 text-capitalize">{{ \App\CPU\Helpers::translate('Withdraw Request Table')}}
                        ...
                        <select name="withdraw_status_filter" onchange="status_filter(this.value)" class="custom-select max-w-200">
                            ...
                            <option value="approved" {{session()->has('withdraw_status_filter') && session('withdraw_status_filter') == 'approved'?'selected':''}}>{{\App\CPU\Helpers::translate('Approved')}}</option>
                            <option value="denied" {{session()->has('withdraw_status_filter') && session('withdraw_status_filter') == 'denied'?'selected':''}}>{{\App\CPU\Helpers::translate('Denied')}}</option>
                            <option value="pending" {{session()->has('withdraw_status_filter') && session('withdraw_status_filter') == 'pending'?'selected':''}}>{{\App\CPU\Helpers::translate('Pending')}}</option>
                        </select>
                    </div>
                            ...
                            @if($withdraw_requests->count() > 0)
                                @foreach($withdraw_requests as $key=>$withdraw_request)
                                    <tr>
                                        ...
                                        <td>{{\App\CPU\BackEndHelper::set_symbol(($withdraw_request['amount']))}}</td>
                                        ...
                                        <td class="text-center">
                                            ...
                                            @else
                                                <span class="btn btn--primary btn-primary btn-sm disabled">
                                                    {{\App\CPU\Helpers::translate('close')}}
                                                </span>
                                            @endif
                                        </td>
  ...
  <script>
      function close_request(route_name) {
          swal({
              ...
              text: "{{\App\CPU\Helpers::translate('Once deleted, you will not be able to recover this')}}",
              icon: "{{\App\CPU\Helpers::translate('warning')}}",
              buttons: true,
              dangerMode: true,
              confirmButtonText: "{{\App\CPU\Helpers::translate('OK')}}",
          })
              .then((willDelete) => {
                  if (willDelete.value) {
