...
@section('title',\App\CPU\Helpers::translate('All Category Page'))
...
@section('content')
    ...
    <div class="container p-3 rtl" style="text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};">
        <div class="row">
            ...
            <div class="col-md-9">
                <h4>{{\App\CPU\Helpers::translate('category')}}</h4>
            </div>
        </div>
        <div class="row">
            <!-- Sidebar-->
            <div class="col-lg-3 col-md-4">
                @foreach(\App\CPU\CategoryManager::parents() as $category)
                    <div class="card-header mb-2 p-2 side-category-bar" onclick="get_categories('{{route('category-ajax',[$category['id']])}}')">
                        <img src="{{asset("storage/app/public/category/".(\App\CPU\Helpers::get_prop('App\Model\Category',$category['id'],'image',session()->get('local')) ?? $category->icon))}}" onerror="this.src='{{asset('public/assets/front-end/img/image-place-holder.png')}}'" style="width: 18px; height: 18px; margin-right: 5px;">
            ...
            <div class="col-lg-9 col-md-8">
                ...
                <div class="row" id="ajax-categories">
                    <label class="col-md-12 text-center mt-5">{{\App\CPU\Helpers::translate('Select your desire category')}}.</label>
                </div>
                <!-- Pagination-->
            </div>
        </div>
