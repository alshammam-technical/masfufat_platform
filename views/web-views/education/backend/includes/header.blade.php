<header class="vironeer-page-header">
    ...
    <div class="button">
        <a href="{{ url('/') }}" target="_blank" class="btn btn-outline-dark"><i
    ...
    <div class="vironeer-notifications ms-2" data-dropdown-v2>
        <div class="vironeer-notifications-title">
            ...
            <div class="counter @if (!$unreadAdminNotifications) bg-secondary @else flashit @endif">{{ $unreadAdminNotifications }}</div>
        </div>

        <div class="vironeer-notifications-menu">
            <div class="vironeer-notifications-header">
                <p class="vironeer-notifications-header-title mb-0">{{ __('Notifications') }}</p>
                ...
                @forelse ($adminNotifications as $adminNotification)
                    @if ($adminNotification->link)
                        <a class="vironeer-notification"
                            href="{{ route('admin.notifications.view', encrypt($adminNotification->id)) }}">
                        @else
        ...
        <a class="vironeer-notifications-footer" href="{{ route('admin.notifications') }}">
            {{ __('View All') }}
        </a>
    </div>
    </div>
