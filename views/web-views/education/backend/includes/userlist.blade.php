<div class="custom-card card mb-3">
    <div class="card-body text-center">
        ...
        <div class="buttons">
            ...
            <form class="d-inline" action="{{ route('users.deleteAvatar', $user->id) }}" method="POST">
                @csrf
                @method('DELETE')
...
<div class="custom-card card mb-3">
    <div class="custom-list-group list-group list-group-flush">
        <a href="{{ route('users.edit', $user->id) }}"
            class="list-group-item list-group-item-action d-flex justify-content-between @if (request()->routeIs('users.edit')) active @endif">
            <span>{{ __('Details') }}</span>
            <span><i class="fas fa-angle-right"></i></span>
        </a>
        <a href="{{ route('users.logs', $user->id) }}"
            class="list-group-item list-group-item-action d-flex justify-content-between @if (request()->routeIs('users.logs')) active @endif">
